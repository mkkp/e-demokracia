...
import AdminAdminView from '~/pages/admin/admin/admin_counties/view/index';
...
export interface PageCreateAdminCountiesFormProps {
  successCallback: (result: AdminCountyStored, open?: boolean) => void;
  cancel: () => void;
}
...
export default function PageCreateAdminCountiesForm({ successCallback, cancel }: PageCreateAdminCountiesFormProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await adminCountyServiceForClassImpl.getTemplate();
  ...
  const submit = async () => {
    ...
    try {
      const res = await adminAdminServiceForAdminCountiesImpl.createAdminCounties(payloadDiff);
  ...
  return (
    <>
      <DialogTitle>
        {title}
        <IconButton
          id="CreateActionedemokraciaAdminAdminEdemokraciaAdminAdminAdminCountiesTableEdemokraciaAdminAdminEdemokraciaAdminAdminAdminCountiesPageCreate-dialog-close"
          aria-label="close"
          onClick={() => cancel()}
          sx={{
            position: 'absolute',
        ...
        <Grid container spacing={2} direction="column" alignItems="stretch" justifyContent="flex-start">
          <Grid item xs={12} sm={12}>
            <TextField
              required={true}
              name="name"
              id="TextInputedemokraciaAdminAdminEdemokraciaAdminAdminAdminCountiesCreateDefaultCountyFormName"
              autoFocus
              label={t('admin.CountyForm.name', { defaultValue: 'County name' }) as string}
              value={data.name ?? ''}
              className={clsx({
      ...
      <DialogActions>
        <Button
          id="CreateActionedemokraciaAdminAdminEdemokraciaAdminAdminAdminCountiesTableEdemokraciaAdminAdminEdemokraciaAdminAdminAdminCountiesPageCreate-action-form-action-cancel"
          variant="text"
          onClick={() => cancel()}
          disabled={isLoading}
        >
        ...
        <ButtonGroup size="small" ref={anchorRef} aria-label="split button" disabled={isLoading}>
          <LoadingButton
            ...
            id="CreateActionedemokraciaAdminAdminEdemokraciaAdminAdminAdminCountiesTableEdemokraciaAdminAdminEdemokraciaAdminAdminAdminCountiesPageCreate-action-form-action-create"
            variant="contained"
            onClick={async () => {
