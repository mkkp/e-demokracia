...
// Page name: edemokracia::admin::Admin.userOwnedYesNoAbstainVoteDefinitions#Table
...
/**
 * Name: edemokracia::admin::Admin.userOwnedYesNoAbstainVoteDefinitions#Table
 * Is Access: true
 * Is Dashboard: false
 * Type: Table
 **/
export default function AdminAdminUserOwnedYesNoAbstainVoteDefinitionsTable() {
  const tableRef = useRef<RefreshableTable>(null);
  const { t } = useTranslation();
  const { locale: l10nLocale } = useL10N();
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const title: string = t('admin.YesNoAbstainVoteDefinitionTable', {
      ...
      <PageContainerTransition>
        <Box sx={mainContainerPadding}>
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Card>
                <CardContent id="PageDefinitionedemokraciaAdminAdminEdemokraciaAdminAdminUserOwnedYesNoAbstainVoteDefinitionsTable-data-grid">
