...
// Page name: edemokracia::admin::UserManager.users#Table
...
/**
 * Name: edemokracia::admin::UserManager.users#Table
 * Is Access: false
 * Type: Table
 **/
export default function AdminUserManagerUsersTable() {
  const tableRef = useRef<RefreshableTable>(null);
  const { t } = useTranslation();
  const { signedIdentifier } = useParams();
  const { locale: l10nLocale } = useL10N();
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const title: string = t('admin.UserTable', { defaultValue: 'User Table' });
      ...
      <PageContainerTransition>
        <Box sx={mainContainerPadding}>
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Card>
                <CardContent id="PageDefinitionedemokraciaAdminAdminEdemokraciaAdminUserManagerUsersTable-data-grid">
                  <User_TableTable
                    ref={tableRef}
