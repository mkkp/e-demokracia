...
export interface TableCreateCitiesFormProps {
  successCallback: (result: AdminCityStored, open?: boolean) => void;
  cancel: () => void;
...
export function TableCreateCitiesForm({ successCallback, cancel, owner }: TableCreateCitiesFormProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await adminCityServiceForClassImpl.getTemplate();
  ...
  return (
    <>
      <DialogTitle>
        {title}
        <IconButton
          id="CreateActionedemokraciaAdminAdminEdemokraciaAdminUserProfileActivityCountiesViewEdemokraciaAdminAdminEdemokraciaAdminCountyCitiesTableCreate-dialog-close"
          aria-label="close"
          onClick={() => cancel()}
          sx={{
            position: 'absolute',
        ...
        <Grid container spacing={2} direction="column" alignItems="stretch" justifyContent="flex-start">
          <Grid item xs={12} sm={12}>
            <TextField
              ...
              autoFocus
              label={t('admin.CityForm.name', { defaultValue: 'City name' }) as string}
              value={data.name ?? ''}
              className={clsx({
      ...
      <DialogActions>
        <Button
          id="CreateActionedemokraciaAdminAdminEdemokraciaAdminUserProfileActivityCountiesViewEdemokraciaAdminAdminEdemokraciaAdminCountyCitiesTableCreate-action-form-action-cancel"
          variant="text"
          onClick={() => cancel()}
          disabled={isLoading}
        >
        ...
        <ButtonGroup size="small" ref={anchorRef} aria-label="split button" disabled={isLoading}>
          <LoadingButton
            ...
            id="CreateActionedemokraciaAdminAdminEdemokraciaAdminUserProfileActivityCountiesViewEdemokraciaAdminAdminEdemokraciaAdminCountyCitiesTableCreate-action-form-action-create"
            variant="contained"
            onClick={async () => {
                  ...
                  <MenuList id="split-button-menu" autoFocusItem>
                    <MenuItem
                      ...
                      onClick={async (event: any) => {
                        const result: AdminCityStored | undefined = await submit();
