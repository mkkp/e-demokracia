...
import { adminIssueAttachmentServiceForClassImpl } from '~/generated/data-axios';
import {} from './actions';
...
export default function AdminAdminView(props: AdminAdminViewProps) {
  ...
  const [data, setData] = useState<AdminIssueAttachmentStored>(
    entry ? { ...entry } : ({} as unknown as AdminIssueAttachmentStored),
  );
  ...
  async function fetchData() {
    ...
    try {
      const res = await adminIssueAttachmentServiceForClassImpl.refresh(
        { __signedIdentifier: entry.__signedIdentifier } as JudoIdentifiable<AdminIssueAttachment>,
