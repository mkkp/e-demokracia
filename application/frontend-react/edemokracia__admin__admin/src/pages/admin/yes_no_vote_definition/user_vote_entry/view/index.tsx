...
// Page name: edemokracia::admin::YesNoVoteDefinition.userVoteEntry#View
...
import { adminYesNoVoteEntryServiceForClassImpl } from '~/generated/data-axios';
import {} from './actions';
...
export type AdminAdminViewPostRefreshAction = (
  data: AdminYesNoVoteEntryStored,
  storeDiff: (attributeName: keyof AdminYesNoVoteEntryStored, value: any) => void,
  setEditMode: Dispatch<SetStateAction<boolean>>,
  setValidation: Dispatch<SetStateAction<Map<keyof AdminYesNoVoteEntry, string>>>,
) => Promise<void>;
...
/**
 * Name: edemokracia::admin::YesNoVoteDefinition.userVoteEntry#View
 * Is Access: false
 * Type: View
...
export default function AdminAdminView(props: AdminAdminViewProps) {
  ...
  const [data, setData] = useState<AdminYesNoVoteEntryStored>(
    entry ? { ...entry } : ({} as unknown as AdminYesNoVoteEntryStored),
  );
  const [payloadDiff, setPayloadDiff] = useState<Record<keyof AdminYesNoVoteEntryStored, any>>(
    {} as unknown as Record<keyof AdminYesNoVoteEntryStored, any>,
  );
  const [editMode, setEditMode] = useState<boolean>(false);
  const storeDiff: (attributeName: keyof AdminYesNoVoteEntryStored, value: any) => void = useCallback(
    (attributeName: keyof AdminYesNoVoteEntryStored, value: any) => {
  ...
  const [validation, setValidation] = useState<Map<keyof AdminYesNoVoteEntry, string>>(
    new Map<keyof AdminYesNoVoteEntry, string>(),
  );
  ...
  const title: string = t('admin.YesNoVoteEntryView', { defaultValue: 'YesNoVoteEntry View / Edit' });
  ...
  async function fetchData() {
    ...
    try {
      const res = await adminYesNoVoteEntryServiceForClassImpl.refresh(
        { __signedIdentifier: entry.__signedIdentifier } as JudoIdentifiable<AdminYesNoVoteEntry>,
      ...
      } as Record<keyof AdminYesNoVoteEntryStored, any>);
        ...
        <Grid container spacing={2} direction="column" alignItems="stretch" justifyContent="flex-start">
          <Grid item xs={12} sm={12}>
            <DateTimePicker
              ...
              slotProps={{
                textField: {
                  id: 'DateTimeInputedemokraciaAdminAdminEdemokraciaAdminYesNoVoteDefinitionUserVoteEntryViewDefaultYesNoVoteEntryViewEditCreated',
                  required: true,
              ...
              onError={(newError: DateTimeValidationError, value: any) => {
                ...
                setValidation((prevValidation) => {
                  const copy = new Map<keyof AdminYesNoVoteEntry, string>(prevValidation);
                  copy.set(
                    'created',
              ...
              label={t('admin.YesNoVoteEntryView.created', { defaultValue: 'Created' }) as string}
          ...
          <Grid item xs={12} sm={12}>
            <TextField
              required={true}
              name="value"
              id="EnumerationComboedemokraciaAdminAdminEdemokraciaAdminYesNoVoteDefinitionUserVoteEntryViewDefaultYesNoVoteEntryViewEditValue"
              label={t('admin.YesNoVoteEntryView.value', { defaultValue: 'Value' }) as string}
              value={data.value || ''}
              className={clsx({
                'JUDO-viewMode': !editMode,
                'JUDO-required': true,
