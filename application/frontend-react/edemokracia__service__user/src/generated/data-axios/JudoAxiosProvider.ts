...
export class JudoAxiosProvider implements AxiosProvider {
  ...
  getBasePath(suffix?: string): string {
    let base = typeof this.basePathFactory !== 'function' ? '/edemokracia::service::User' : this.basePathFactory();
    base = base.replace(/\/+$/, '');
  ...
  getFilePath(suffix?: string): string {
    let base = typeof this.filePathFactory !== 'function' ? '/edemokracia::service::User' : this.filePathFactory();
    base = base.replace(/\/+$/, '');
