...
import type { ServiceCommentServiceForClass } from '../data-service';

/**
 * Class Service Implementation for ServiceComment
 */
export class ServiceCommentServiceForClassImpl extends JudoAxiosService implements ServiceCommentServiceForClass {
  /**
   ...
   */
  async delete(target: JudoIdentifiable<ServiceComment>): Promise<void> {
    const path = '/service/Comment/~delete';
   ...
   * @throws {AxiosError} With data containing {@link Array<FeedbackItem>} for status codes: 400, 401, 403.
   */
  async update(target: Partial<ServiceCommentStored>): Promise<ServiceCommentStored> {
    const path = '/service/Comment/~update';
