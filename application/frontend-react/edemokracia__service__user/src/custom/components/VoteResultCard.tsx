import { Box, Button, Card, CardContent, CardHeader, Chip, Typography, useTheme } from '@mui/material';
...
const VoteResultsCard: React.FC<VoteResultsCardProps> = ({
  ...
  function answerColor(voteValue: string | undefined): string | undefined {
    ...
    const ret = processedAnswers.filter((answer) => answer.label == voteValue);
    if (ret.length > 0) {
      return ret[0].color;
    } else {
      return undefined;
