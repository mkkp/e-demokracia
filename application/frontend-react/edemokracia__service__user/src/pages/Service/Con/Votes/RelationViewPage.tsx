...
export const SERVICE_CON_VOTES_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_CON_VOTES_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceConVotesRelationViewPagePayload = (
...
// XMIID: User/(esm/_qZ3sEGksEe25ONJ3V89cVA)/RelationFeatureView
// Name: service::Con::votes::RelationViewPage
export default function ServiceConVotesRelationViewPage() {
  ...
  const storeDiff: (attributeName: keyof ServiceSimpleVote, value: any) => void = useCallback(
    (attributeName: keyof ServiceSimpleVote, value: any) => {
      payloadDiff.current![attributeName] = convertServiceConVotesRelationViewPagePayload(attributeName, value);
      setData((prevData) => ({
        ...prevData,
        [attributeName]: value,
      }));
  ...
  const { service: customActionsHook } = useTrackService<ServiceSimpleVoteSimpleVote_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_CON_VOTES_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceConServiceForVotesImpl.refresh(
  ...
  return (
    <ServiceSimpleVoteSimpleVote_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_qZ3sEGksEe25ONJ3V89cVA)/RelationFeatureView"
          data-page-name="service::Con::votes::RelationViewPage"
        />
