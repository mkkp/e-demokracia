...
import { useServiceUserIssuesActiveIssuesGlobalAddSelectorPage } from '~/dialogs/Service/UserIssues/ActiveIssuesGlobal/AddSelectorPage';
...
export const SERVICE_USER_ISSUES_ACTIVE_ISSUES_GLOBAL_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
...
// XMIID: User/(esm/_NM0voFrKEe6_67aMO2jOsw)/RelationFeatureTable
// Name: service::UserIssues::activeIssuesGlobal::RelationTablePage
export default function ServiceUserIssuesActiveIssuesGlobalRelationTablePage() {
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_TableActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_ACTIVE_ISSUES_GLOBAL_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const openServiceUserIssuesActiveIssuesGlobalAddSelectorPage =
    useServiceUserIssuesActiveIssuesGlobalAddSelectorPage();
  ...
  const activateForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesGlobalImpl.activate(target!);
  ...
  const addToFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesGlobalImpl.addToFavorites(target!);
  ...
  const closeVoteForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesGlobalImpl.closeVote(target!);
  ...
  const deleteOrArchiveForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesGlobalImpl.deleteOrArchive(target!);
  ...
  const removeFromFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesGlobalImpl.removeFromFavorites(target!);
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } = await openServiceUserIssuesActiveIssuesGlobalAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceUserIssuesServiceForActiveIssuesGlobalImpl.addActiveIssuesGlobal(
  ...
  const removeAction = async (target?: ServiceIssueStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceUserIssuesServiceForActiveIssuesGlobalImpl.removeActiveIssuesGlobal(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
  ...
  const refreshAction = async (queryCustomizer: ServiceIssueQueryCustomizer): Promise<ServiceIssueStored[]> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      return serviceUserIssuesServiceForActiveIssuesGlobalImpl.list(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
        queryCustomizer,
      );
    } catch (error) {
  ...
  const openPageAction = async (target?: ServiceIssueStored) => {
    ...
    navigate(routeToServiceUserIssuesActiveIssuesGlobalRelationViewPage(target!.__signedIdentifier));
  };
  ...
  return (
    <div
      id="User/(esm/_NM0voFrKEe6_67aMO2jOsw)/RelationFeatureTable"
      data-page-name="service::UserIssues::activeIssuesGlobal::RelationTablePage"
    >
      <Suspense>
