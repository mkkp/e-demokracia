...
import { ServiceUserIssuesServiceForActiveIssuesGlobalImpl } from '~/services/data-axios/ServiceUserIssuesServiceForActiveIssuesGlobalImpl';
...
export const SERVICE_USER_ISSUES_ACTIVE_ISSUES_GLOBAL_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_ISSUES_ACTIVE_ISSUES_GLOBAL_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceUserIssuesActiveIssuesGlobalRelationViewPagePayload = (
...
// XMIID: User/(esm/_NM0voFrKEe6_67aMO2jOsw)/RelationFeatureView
// Name: service::UserIssues::activeIssuesGlobal::RelationViewPage
export default function ServiceUserIssuesActiveIssuesGlobalRelationViewPage() {
  // Router params section
  const { signedIdentifier } = useParams();

  // Services
  const serviceUserIssuesServiceForActiveIssuesGlobalImpl = useMemo(
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] = convertServiceUserIssuesActiveIssuesGlobalRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_ACTIVE_ISSUES_GLOBAL_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesGlobalImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesGlobalImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesGlobalImpl.closeVote(data);
  ...
  const deleteOrArchiveForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesGlobalImpl.deleteOrArchive(data);
  ...
  const removeFromFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesGlobalImpl.removeFromFavorites(data);
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      return serviceUserIssuesServiceForActiveIssuesGlobalImpl.getRangeForIssueType(
        cleanUpPayload(data),
        queryCustomizer,
      );
    } catch (error) {
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      return serviceUserIssuesServiceForActiveIssuesGlobalImpl.getRangeForOwner(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      return serviceUserIssuesServiceForActiveIssuesGlobalImpl.getRangeForCity(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      return serviceUserIssuesServiceForActiveIssuesGlobalImpl.getRangeForCounty(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      return serviceUserIssuesServiceForActiveIssuesGlobalImpl.getRangeForDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
    } catch (error) {
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesGlobalImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesGlobalImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesGlobalImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const refreshAction = async (queryCustomizer: ServiceIssueQueryCustomizer): Promise<ServiceIssueStored> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceUserIssuesServiceForActiveIssuesGlobalImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const res = await serviceUserIssuesServiceForActiveIssuesGlobalImpl.update(payloadDiff.current);
      if (res) {
  ...
  return (
    <ServiceIssueIssue_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_NM0voFrKEe6_67aMO2jOsw)/RelationFeatureView"
          data-page-name="service::UserIssues::activeIssuesGlobal::RelationViewPage"
        />
