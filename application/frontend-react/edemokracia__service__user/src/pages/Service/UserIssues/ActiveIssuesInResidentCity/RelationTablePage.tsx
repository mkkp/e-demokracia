...
import { useServiceUserIssuesActiveIssuesInResidentCityAddSelectorPage } from '~/dialogs/Service/UserIssues/ActiveIssuesInResidentCity/AddSelectorPage';
...
export const SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_RESIDENT_CITY_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
...
// XMIID: User/(esm/_T07cUFrbEe6gN-oVBDDIOQ)/RelationFeatureTable
// Name: service::UserIssues::activeIssuesInResidentCity::RelationTablePage
export default function ServiceUserIssuesActiveIssuesInResidentCityRelationTablePage() {
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_TableActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_RESIDENT_CITY_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const openServiceUserIssuesActiveIssuesInResidentCityAddSelectorPage =
    useServiceUserIssuesActiveIssuesInResidentCityAddSelectorPage();
  ...
  const activateForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.activate(target!);
  ...
  const addToFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.addToFavorites(target!);
  ...
  const closeVoteForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.closeVote(target!);
  ...
  const deleteOrArchiveForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.deleteOrArchive(target!);
  ...
  const removeFromFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.removeFromFavorites(target!);
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } = await openServiceUserIssuesActiveIssuesInResidentCityAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.addActiveIssuesInResidentCity(
  ...
  const removeAction = async (target?: ServiceIssueStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.removeActiveIssuesInResidentCity(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
  ...
  const refreshAction = async (queryCustomizer: ServiceIssueQueryCustomizer): Promise<ServiceIssueStored[]> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      return serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.list(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
        queryCustomizer,
      );
    } catch (error) {
  ...
  const openPageAction = async (target?: ServiceIssueStored) => {
    ...
    navigate(routeToServiceUserIssuesActiveIssuesInResidentCityRelationViewPage(target!.__signedIdentifier));
  };
  ...
  return (
    <div
      id="User/(esm/_T07cUFrbEe6gN-oVBDDIOQ)/RelationFeatureTable"
      data-page-name="service::UserIssues::activeIssuesInResidentCity::RelationTablePage"
    >
      <Suspense>
