...
import { ServiceUserIssuesServiceForActiveIssuesInResidentCityImpl } from '~/services/data-axios/ServiceUserIssuesServiceForActiveIssuesInResidentCityImpl';
...
export const SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_RESIDENT_CITY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_RESIDENT_CITY_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceUserIssuesActiveIssuesInResidentCityRelationViewPagePayload = (
...
// XMIID: User/(esm/_T07cUFrbEe6gN-oVBDDIOQ)/RelationFeatureView
// Name: service::UserIssues::activeIssuesInResidentCity::RelationViewPage
export default function ServiceUserIssuesActiveIssuesInResidentCityRelationViewPage() {
  // Router params section
  const { signedIdentifier } = useParams();

  // Services
  const serviceUserIssuesServiceForActiveIssuesInResidentCityImpl = useMemo(
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] = convertServiceUserIssuesActiveIssuesInResidentCityRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_RESIDENT_CITY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.closeVote(data);
  ...
  const deleteOrArchiveForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.deleteOrArchive(data);
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.getRangeForIssueType(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.getRangeForOwner(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.getRangeForCity(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.getRangeForCounty(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.getRangeForDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const removeFromFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.removeFromFavorites(data);
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceUserIssuesServiceForActiveIssuesInResidentCityImpl.update(payloadDiff.current);
      if (res) {
  ...
  return (
    <ServiceIssueIssue_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_T07cUFrbEe6gN-oVBDDIOQ)/RelationFeatureView"
          data-page-name="service::UserIssues::activeIssuesInResidentCity::RelationViewPage"
        />
