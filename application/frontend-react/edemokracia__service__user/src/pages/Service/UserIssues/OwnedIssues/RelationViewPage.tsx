...
export const SERVICE_USER_ISSUES_OWNED_ISSUES_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_ISSUES_OWNED_ISSUES_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceUserIssuesOwnedIssuesRelationViewPagePayload = (
...
// XMIID: User/(esm/_jK5OoVq4Ee6_67aMO2jOsw)/RelationFeatureView
// Name: service::UserIssues::ownedIssues::RelationViewPage
export default function ServiceUserIssuesOwnedIssuesRelationViewPage() {
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] = convertServiceUserIssuesOwnedIssuesRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_OWNED_ISSUES_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForOwnedIssuesImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForOwnedIssuesImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForOwnedIssuesImpl.closeVote(data);
  ...
  const deleteOrArchiveForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForOwnedIssuesImpl.deleteOrArchive(data);
  ...
  const removeFromFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForOwnedIssuesImpl.removeFromFavorites(data);
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      return serviceUserIssuesServiceForOwnedIssuesImpl.getRangeForIssueType(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      return serviceUserIssuesServiceForOwnedIssuesImpl.getRangeForOwner(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      return serviceUserIssuesServiceForOwnedIssuesImpl.getRangeForCity(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      return serviceUserIssuesServiceForOwnedIssuesImpl.getRangeForCounty(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      return serviceUserIssuesServiceForOwnedIssuesImpl.getRangeForDistrict(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForOwnedIssuesImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForOwnedIssuesImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForOwnedIssuesImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const refreshAction = async (queryCustomizer: ServiceIssueQueryCustomizer): Promise<ServiceIssueStored> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceUserIssuesServiceForOwnedIssuesImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const res = await serviceUserIssuesServiceForOwnedIssuesImpl.update(payloadDiff.current);
      if (res) {
  ...
  return (
    <ServiceIssueIssue_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_jK5OoVq4Ee6_67aMO2jOsw)/RelationFeatureView"
          data-page-name="service::UserIssues::ownedIssues::RelationViewPage"
        />
