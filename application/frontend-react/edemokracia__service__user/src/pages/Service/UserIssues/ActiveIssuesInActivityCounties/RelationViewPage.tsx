...
export const SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_ACTIVITY_COUNTIES_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_ACTIVITY_COUNTIES_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceUserIssuesActiveIssuesInActivityCountiesRelationViewPagePayload = (
...
// XMIID: User/(esm/_aMsB4FrWEe6gN-oVBDDIOQ)/RelationFeatureView
// Name: service::UserIssues::activeIssuesInActivityCounties::RelationViewPage
export default function ServiceUserIssuesActiveIssuesInActivityCountiesRelationViewPage() {
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] =
        convertServiceUserIssuesActiveIssuesInActivityCountiesRelationViewPagePayload(attributeName, value);
      setData((prevData) => ({
        ...prevData,
        [attributeName]: value,
      }));
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_ACTIVITY_COUNTIES_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.closeVote(data);
  ...
  const deleteOrArchiveForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.deleteOrArchive(data);
  ...
  const removeFromFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.removeFromFavorites(data);
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.getRangeForIssueType(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.getRangeForOwner(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.getRangeForCity(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.getRangeForCounty(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.getRangeForDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceUserIssuesServiceForActiveIssuesInActivityCountiesImpl.update(
        payloadDiff.current,
      );
      if (res) {
  ...
  return (
    <ServiceIssueIssue_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_aMsB4FrWEe6gN-oVBDDIOQ)/RelationFeatureView"
          data-page-name="service::UserIssues::activeIssuesInActivityCounties::RelationViewPage"
        />
