...
export const SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_ACTIVITY_DISTRICTS_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
...
// XMIID: User/(esm/_riB9EFrWEe6gN-oVBDDIOQ)/RelationFeatureTable
// Name: service::UserIssues::activeIssuesInActivityDistricts::RelationTablePage
export default function ServiceUserIssuesActiveIssuesInActivityDistrictsRelationTablePage() {
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_TableActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_ACTIVITY_DISTRICTS_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const addToFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.addToFavorites(target!);
  ...
  const deleteOrArchiveForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.deleteOrArchive(target!);
  ...
  const refreshAction = async (queryCustomizer: ServiceIssueQueryCustomizer): Promise<ServiceIssueStored[]> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      return serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.list(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
        queryCustomizer,
      );
    } catch (error) {
  ...
  const openPageAction = async (target?: ServiceIssueStored) => {
    ...
    navigate(routeToServiceUserIssuesActiveIssuesInActivityDistrictsRelationViewPage(target!.__signedIdentifier));
  };
  ...
  return (
    <div
      id="User/(esm/_riB9EFrWEe6gN-oVBDDIOQ)/RelationFeatureTable"
      data-page-name="service::UserIssues::activeIssuesInActivityDistricts::RelationTablePage"
    >
      <Suspense>
