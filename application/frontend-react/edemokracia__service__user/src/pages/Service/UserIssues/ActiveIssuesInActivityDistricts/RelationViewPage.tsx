...
import { ServiceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl } from '~/services/data-axios/ServiceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl';
...
export const SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_ACTIVITY_DISTRICTS_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_ACTIVITY_DISTRICTS_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceUserIssuesActiveIssuesInActivityDistrictsRelationViewPagePayload = (
...
// XMIID: User/(esm/_riB9EFrWEe6gN-oVBDDIOQ)/RelationFeatureView
// Name: service::UserIssues::activeIssuesInActivityDistricts::RelationViewPage
export default function ServiceUserIssuesActiveIssuesInActivityDistrictsRelationViewPage() {
  // Router params section
  const { signedIdentifier } = useParams();

  // Services
  const serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl = useMemo(
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] =
        convertServiceUserIssuesActiveIssuesInActivityDistrictsRelationViewPagePayload(attributeName, value);
      setData((prevData) => ({
        ...prevData,
        [attributeName]: value,
      }));
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_ACTIVITY_DISTRICTS_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.closeVote(data);
  ...
  const deleteOrArchiveForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.deleteOrArchive(data);
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } =
        await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.getRangeForIssueType(
          cleanUpPayload(data),
          queryCustomizer,
        );
      return result;
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.getRangeForOwner(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.getRangeForCity(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.getRangeForCounty(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.getRangeForDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const removeFromFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.removeFromFavorites(data);
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceUserIssuesServiceForActiveIssuesInActivityDistrictsImpl.update(
        payloadDiff.current,
      );
      if (res) {
  ...
  return (
    <ServiceIssueIssue_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_riB9EFrWEe6gN-oVBDDIOQ)/RelationFeatureView"
          data-page-name="service::UserIssues::activeIssuesInActivityDistricts::RelationViewPage"
        />
