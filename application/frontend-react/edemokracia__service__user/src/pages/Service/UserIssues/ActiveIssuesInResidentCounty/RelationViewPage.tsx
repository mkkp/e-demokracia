...
export const SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceUserIssuesActiveIssuesInResidentCountyRelationViewPagePayload = (
...
// XMIID: User/(esm/_KOPO4FrbEe6gN-oVBDDIOQ)/RelationFeatureView
// Name: service::UserIssues::activeIssuesInResidentCounty::RelationViewPage
export default function ServiceUserIssuesActiveIssuesInResidentCountyRelationViewPage() {
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] = convertServiceUserIssuesActiveIssuesInResidentCountyRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.closeVote(data);
  ...
  const deleteOrArchiveForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.deleteOrArchive(data);
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.getRangeForIssueType(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.getRangeForOwner(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.getRangeForCity(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.getRangeForCounty(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.getRangeForDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const removeFromFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.removeFromFavorites(data);
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceUserIssuesServiceForActiveIssuesInResidentCountyImpl.update(
        payloadDiff.current,
      );
      if (res) {
  ...
  return (
    <ServiceIssueIssue_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_KOPO4FrbEe6gN-oVBDDIOQ)/RelationFeatureView"
          data-page-name="service::UserIssues::activeIssuesInResidentCounty::RelationViewPage"
        />
