...
import { useServiceUserIssuesActiveIssuesInActivityCitiesAddSelectorPage } from '~/dialogs/Service/UserIssues/ActiveIssuesInActivityCities/AddSelectorPage';
...
// XMIID: User/(esm/_jlpVUFrWEe6gN-oVBDDIOQ)/RelationFeatureTable
// Name: service::UserIssues::activeIssuesInActivityCities::RelationTablePage
export default function ServiceUserIssuesActiveIssuesInActivityCitiesRelationTablePage() {
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_TableActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_ACTIVE_ISSUES_IN_ACTIVITY_CITIES_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const openServiceUserIssuesActiveIssuesInActivityCitiesAddSelectorPage =
    useServiceUserIssuesActiveIssuesInActivityCitiesAddSelectorPage();
  ...
  const activateForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityCitiesImpl.activate(target!);
  ...
  const addToFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityCitiesImpl.addToFavorites(target!);
  ...
  const closeVoteForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityCitiesImpl.closeVote(target!);
  ...
  const deleteOrArchiveForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityCitiesImpl.deleteOrArchive(target!);
  ...
  const removeFromFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceUserIssuesServiceForActiveIssuesInActivityCitiesImpl.removeFromFavorites(target!);
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } = await openServiceUserIssuesActiveIssuesInActivityCitiesAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceUserIssuesServiceForActiveIssuesInActivityCitiesImpl.addActiveIssuesInActivityCities(
  ...
  const removeAction = async (target?: ServiceIssueStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceUserIssuesServiceForActiveIssuesInActivityCitiesImpl.removeActiveIssuesInActivityCities(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
  ...
  const refreshAction = async (queryCustomizer: ServiceIssueQueryCustomizer): Promise<ServiceIssueStored[]> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      return serviceUserIssuesServiceForActiveIssuesInActivityCitiesImpl.list(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
        queryCustomizer,
      );
    } catch (error) {
