...
export const SERVICE_USER_ISSUES_USER_ISSUES_VIEW_EDIT_CREATE_ISSUE_OUTPUT_VIEW_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_ISSUES_USER_ISSUES_VIEW_EDIT_CREATE_ISSUE_OUTPUT_VIEW_ACTIONS_HOOK';
...
// XMIID: User/(esm/_jK51w1q4Ee6_67aMO2jOsw)/OperationOutputPageDefinition
// Name: service::UserIssues::UserIssues_View_Edit::createIssue::Output::View
export default function ServiceUserIssuesUserIssues_View_EditCreateIssueOutputView() {
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] = convertServiceUserIssuesUserIssues_View_EditCreateIssueOutputViewPayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceIssueServiceImpl.getRangeForIssueType(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceIssueServiceImpl.getRangeForOwner(cleanUpPayload(data), queryCustomizer);
      return result;
    } catch (error: any) {
      handleError(error);
      return Promise.resolve([]);
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceIssueServiceImpl.getRangeForCity(cleanUpPayload(data), queryCustomizer);
      return result;
    } catch (error: any) {
      handleError(error);
      return Promise.resolve([]);
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceIssueServiceImpl.getRangeForCounty(cleanUpPayload(data), queryCustomizer);
      return result;
    } catch (error: any) {
      handleError(error);
      return Promise.resolve([]);
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceIssueServiceImpl.getRangeForDistrict(cleanUpPayload(data), queryCustomizer);
      return result;
    } catch (error: any) {
      handleError(error);
      return Promise.resolve([]);
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceIssueServiceImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceIssueServiceImpl.update(payloadDiff.current);
      if (res) {
