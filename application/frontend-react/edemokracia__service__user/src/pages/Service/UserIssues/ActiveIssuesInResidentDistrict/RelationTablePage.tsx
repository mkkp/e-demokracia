...
import { useServiceUserIssuesActiveIssuesInResidentDistrictAddSelectorPage } from '~/dialogs/Service/UserIssues/ActiveIssuesInResidentDistrict/AddSelectorPage';
...
export default function ServiceUserIssuesActiveIssuesInResidentDistrictRelationTablePage() {
  ...
  const openServiceUserIssuesActiveIssuesInResidentDistrictAddSelectorPage =
    useServiceUserIssuesActiveIssuesInResidentDistrictAddSelectorPage();
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } = await openServiceUserIssuesActiveIssuesInResidentDistrictAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceUserIssuesServiceForActiveIssuesInResidentDistrictImpl.addActiveIssuesInResidentDistrict(
  ...
  const removeAction = async (target?: ServiceIssueStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceUserIssuesServiceForActiveIssuesInResidentDistrictImpl.removeActiveIssuesInResidentDistrict(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
