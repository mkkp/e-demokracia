...
export const SERVICE_DASHBOARD_FAVORITE_ISSUES_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_DASHBOARD_FAVORITE_ISSUES_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
// Name: service::Dashboard::favoriteIssues::RelationViewPage
export default function ServiceDashboardFavoriteIssuesRelationViewPage() {
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceDashboardServiceForFavoriteIssuesImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceDashboardServiceForFavoriteIssuesImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceDashboardServiceForFavoriteIssuesImpl.closeVote(data);
  ...
  const deleteOrArchiveForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceDashboardServiceForFavoriteIssuesImpl.deleteOrArchive(data);
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceDashboardServiceForFavoriteIssuesImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceDashboardServiceForFavoriteIssuesImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceDashboardServiceForFavoriteIssuesImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const removeFromFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceDashboardServiceForFavoriteIssuesImpl.removeFromFavorites(data);
    ...
    <ServiceIssueIssue_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          ...
          data-page-name="service::Dashboard::favoriteIssues::RelationViewPage"
        />
