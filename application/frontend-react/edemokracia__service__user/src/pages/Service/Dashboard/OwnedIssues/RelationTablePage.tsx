...
import { useServiceDashboardOwnedIssuesAddSelectorPage } from '~/dialogs/Service/Dashboard/OwnedIssues/AddSelectorPage';
...
export const SERVICE_DASHBOARD_OWNED_ISSUES_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_DASHBOARD_OWNED_ISSUES_RELATION_TABLE_PAGE_ACTIONS_HOOK';
...
// Name: service::Dashboard::ownedIssues::RelationTablePage
export default function ServiceDashboardOwnedIssuesRelationTablePage() {
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_TableActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_DASHBOARD_OWNED_ISSUES_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  const customActions: ServiceIssueIssue_TablePageActionsExtended | undefined = customActionsHook?.(data, editMode);

  // Dialog hooks
  const openServiceDashboardOwnedIssuesAddSelectorPage = useServiceDashboardOwnedIssuesAddSelectorPage();
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } = await openServiceDashboardOwnedIssuesAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceDashboardServiceForOwnedIssuesImpl.addOwnedIssues(
  ...
  const removeAction = async (target?: ServiceIssueStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceDashboardServiceForOwnedIssuesImpl.removeOwnedIssues(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      return serviceDashboardServiceForOwnedIssuesImpl.list(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
        queryCustomizer,
      );
    } catch (error) {
  ...
  const openPageAction = async (target: ServiceIssue | ServiceIssueStored, isDraft?: boolean) => {
    if (isDraft && (!target || !(target as ServiceIssueStored).__signedIdentifier)) {
      ...
      navigate(routeToServiceDashboardOwnedIssuesRelationViewPage((target as ServiceIssueStored)!.__signedIdentifier));
    }
  };
  const activateForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceDashboardServiceForOwnedIssuesImpl.activate(target!);
  ...
  const addToFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceDashboardServiceForOwnedIssuesImpl.addToFavorites(target!);
  ...
  const closeVoteForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceDashboardServiceForOwnedIssuesImpl.closeVote(target!);
  ...
  const deleteOrArchiveForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await serviceDashboardServiceForOwnedIssuesImpl.deleteOrArchive(target!);
    ...
    <ServiceIssueIssue_TableViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          ...
          data-page-name="service::Dashboard::ownedIssues::RelationTablePage"
        />
