...
import { useServiceDashboardIssuesOwnedAddSelectorPage } from '~/dialogs/Service/Dashboard/IssuesOwned/AddSelectorPage';
...
export default function ServiceDashboardIssuesOwnedRelationTablePage() {
  ...
  // Dialog hooks
  const openServiceDashboardIssuesOwnedAddSelectorPage = useServiceDashboardIssuesOwnedAddSelectorPage();
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } = await openServiceDashboardIssuesOwnedAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceDashboardServiceForIssuesOwnedImpl.addIssuesOwned(
  ...
  const removeAction = async (target?: ServiceIssueStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceDashboardServiceForIssuesOwnedImpl.removeIssuesOwned(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
