...
export const SERVICE_SELECT_ANSWER_VOTE_DEFINITION_ISSUE_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_SELECT_ANSWER_VOTE_DEFINITION_ISSUE_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceSelectAnswerVoteDefinitionIssueRelationViewPagePayload = (
...
// XMIID: User/(esm/_iPXSkFpMEe6_67aMO2jOsw)/RelationFeatureView
// Name: service::SelectAnswerVoteDefinition::issue::RelationViewPage
export default function ServiceSelectAnswerVoteDefinitionIssueRelationViewPage() {
  // Router params section
  const { signedIdentifier } = useParams();

  // Services
  const serviceSelectAnswerVoteDefinitionServiceForIssueImpl = useMemo(
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] = convertServiceSelectAnswerVoteDefinitionIssueRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_SELECT_ANSWER_VOTE_DEFINITION_ISSUE_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceSelectAnswerVoteDefinitionServiceForIssueImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceSelectAnswerVoteDefinitionServiceForIssueImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceSelectAnswerVoteDefinitionServiceForIssueImpl.closeVote(data);
  ...
  const deleteOrArchiveForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceSelectAnswerVoteDefinitionServiceForIssueImpl.deleteOrArchive(data);
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceSelectAnswerVoteDefinitionServiceForIssueImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceSelectAnswerVoteDefinitionServiceForIssueImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceSelectAnswerVoteDefinitionServiceForIssueImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const removeFromFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceSelectAnswerVoteDefinitionServiceForIssueImpl.removeFromFavorites(data);
  ...
  return (
    <ServiceIssueIssue_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_iPXSkFpMEe6_67aMO2jOsw)/RelationFeatureView"
          data-page-name="service::SelectAnswerVoteDefinition::issue::RelationViewPage"
        />
