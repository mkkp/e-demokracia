...
import { ServiceVoteDefinitionServiceForIssueImpl } from '~/services/data-axios/ServiceVoteDefinitionServiceForIssueImpl';
...
export const SERVICE_VOTE_DEFINITION_ISSUE_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_VOTE_DEFINITION_ISSUE_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export default function ServiceVoteDefinitionIssueRelationViewPage() {
  ...
  // Services
  const serviceVoteDefinitionServiceForIssueImpl = useMemo(
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceVoteDefinitionServiceForIssueImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceVoteDefinitionServiceForIssueImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceVoteDefinitionServiceForIssueImpl.closeVote(data);
  ...
  const deleteOrArchiveForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceVoteDefinitionServiceForIssueImpl.deleteOrArchive(data);
  ...
  const removeFromFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceVoteDefinitionServiceForIssueImpl.removeFromFavorites(data);
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      return serviceVoteDefinitionServiceForIssueImpl.getRangeForIssueType(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      return serviceVoteDefinitionServiceForIssueImpl.getRangeForOwner(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      return serviceVoteDefinitionServiceForIssueImpl.getRangeForCity(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      return serviceVoteDefinitionServiceForIssueImpl.getRangeForCounty(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      return serviceVoteDefinitionServiceForIssueImpl.getRangeForDistrict(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceVoteDefinitionServiceForIssueImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceVoteDefinitionServiceForIssueImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceVoteDefinitionServiceForIssueImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const refreshAction = async (queryCustomizer: ServiceIssueQueryCustomizer): Promise<ServiceIssueStored> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceVoteDefinitionServiceForIssueImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
