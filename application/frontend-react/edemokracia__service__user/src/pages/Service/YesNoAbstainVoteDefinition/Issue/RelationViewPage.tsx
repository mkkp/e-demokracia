...
import { ServiceYesNoAbstainVoteDefinitionServiceForIssueImpl } from '~/services/data-axios/ServiceYesNoAbstainVoteDefinitionServiceForIssueImpl';
...
export const SERVICE_YES_NO_ABSTAIN_VOTE_DEFINITION_ISSUE_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_YES_NO_ABSTAIN_VOTE_DEFINITION_ISSUE_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceYesNoAbstainVoteDefinitionIssueRelationViewPagePayload = (
...
// XMIID: User/(esm/_J49TwFodEe6_67aMO2jOsw)/RelationFeatureView
// Name: service::YesNoAbstainVoteDefinition::issue::RelationViewPage
export default function ServiceYesNoAbstainVoteDefinitionIssueRelationViewPage() {
  // Router params section
  const { signedIdentifier } = useParams();

  // Services
  const serviceYesNoAbstainVoteDefinitionServiceForIssueImpl = useMemo(
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] = convertServiceYesNoAbstainVoteDefinitionIssueRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_YES_NO_ABSTAIN_VOTE_DEFINITION_ISSUE_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.closeVote(data);
  ...
  const deleteOrArchiveForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.deleteOrArchive(data);
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.getRangeForIssueType(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.getRangeForOwner(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.getRangeForCity(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.getRangeForCounty(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.getRangeForDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const removeFromFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.removeFromFavorites(data);
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceYesNoAbstainVoteDefinitionServiceForIssueImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
  ...
  return (
    <ServiceIssueIssue_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_J49TwFodEe6_67aMO2jOsw)/RelationFeatureView"
          data-page-name="service::YesNoAbstainVoteDefinition::issue::RelationViewPage"
        />
