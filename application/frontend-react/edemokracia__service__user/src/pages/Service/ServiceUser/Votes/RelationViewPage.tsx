...
export const SERVICE_SERVICE_USER_VOTES_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_SERVICE_USER_VOTES_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceServiceUserVotesRelationViewPagePayload = (
...
// Name: service::ServiceUser::votes::RelationViewPage
export default function ServiceServiceUserVotesRelationViewPage() {
  ...
  const storeDiff: (attributeName: keyof ServiceSimpleVote, value: any) => void = useCallback(
    (attributeName: keyof ServiceSimpleVote, value: any) => {
      payloadDiff.current![attributeName] = convertServiceServiceUserVotesRelationViewPagePayload(attributeName, value);
      setData((prevData) => ({
        ...prevData,
        [attributeName]: value,
      }));
  ...
  const { service: customActionsHook } = useTrackService<ServiceSimpleVoteSimpleVote_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_SERVICE_USER_VOTES_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceServiceUserServiceForVotesImpl.refresh(
  ...
  return (
    <ServiceSimpleVoteSimpleVote_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_qcneEGksEe25ONJ3V89cVA)/RelationFeatureView"
          data-page-name="service::ServiceUser::votes::RelationViewPage"
        />
