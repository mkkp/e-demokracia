...
export const convertServiceUserUserOwnedActiveIssuesAccessViewPagePayload = (
...
// XMIID: User/(esm/_LY9IoFqmEe6_67aMO2jOsw)/AccessViewPageDefinition
// Name: service::User::userOwnedActiveIssues::Access::View::Page
export default function ServiceUserUserOwnedActiveIssuesAccessViewPage() {
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] = convertServiceUserUserOwnedActiveIssuesAccessViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const serviceIssueIssue_View_EditRefresh = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await userServiceForUserOwnedActiveIssuesImpl.refresh(
  ...
  const serviceIssueIssue_View_EditUpdate = async () => {
    ...
    try {
      const res = await userServiceForUserOwnedActiveIssuesImpl.update(payloadDiff.current);
  ...
  const serviceIssueIssue_View_EditActionsPageActionButtonsActivate = async () => {
    try {
      setIsLoading(true);
      await userServiceForUserOwnedActiveIssuesImpl.activate(data);
  ...
  const serviceIssueIssue_View_EditActionsPageActionButtonsCloseVote = async () => {
    try {
      setIsLoading(true);
      await userServiceForUserOwnedActiveIssuesImpl.closeVote(data);
  ...
  const serviceIssueIssue_View_EditActionsPageActionButtonsDeleteOrArchive = async () => {
    try {
      setIsLoading(true);
      await userServiceForUserOwnedActiveIssuesImpl.deleteOrArchive(data);
  ...
  const serviceIssueIssue_View_EditActionsPageActionButtonsRemoveFromFavorites = async () => {
    try {
      setIsLoading(true);
      await userServiceForUserOwnedActiveIssuesImpl.removeFromFavorites(data);
  ...
  const serviceIssueIssue_View_EditActionsPageActionButtonsAddToFavorites = async () => {
    try {
      setIsLoading(true);
      await userServiceForUserOwnedActiveIssuesImpl.addToFavorites(data);
  ...
  const serviceIssueIssue_View_EditIssueOwnerAutocomplete = async (
    ...
    try {
      return userServiceForUserOwnedActiveIssuesImpl.getRangeForOwner(data, queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const serviceIssueIssue_View_EditIssueIssueTypeAutocomplete = async (
    ...
    try {
      return userServiceForUserOwnedActiveIssuesImpl.getRangeForIssueType(data, queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
