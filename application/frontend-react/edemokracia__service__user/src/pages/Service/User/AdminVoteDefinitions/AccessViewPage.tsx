...
// Name: service::User::adminVoteDefinitions::AccessViewPage
export default function ServiceUserAdminVoteDefinitionsAccessViewPage() {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await userServiceForAdminVoteDefinitionsImpl.refresh(
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await userServiceForAdminVoteDefinitionsImpl.update(payloadDiff.current);
      if (res) {
