...
export const SERVICE_USER_USER_CREATED_ISSUES_ACCESS_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
...
// XMIID: User/(esm/_ITVmEFqnEe6_67aMO2jOsw)/AccessTablePageDefinition
// Name: service::User::userCreatedIssues::Access::Table::Page
// Access: true
// Single Access: false
export default function ServiceUserUserCreatedIssuesAccessTablePage() {
  // Hooks section
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_TableActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_USER_CREATED_ISSUES_ACCESS_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const openPageAction = async (target?: ServiceIssueStored) => {
    ...
    navigate(routeToServiceUserUserCreatedIssuesAccessViewPage(target!.__signedIdentifier));
  };
  ...
  const refreshAction = async (queryCustomizer: ServiceIssueQueryCustomizer): Promise<ServiceIssueStored[]> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      return userServiceForUserCreatedIssuesImpl.list(undefined, queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const addToFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await userServiceForUserCreatedIssuesImpl.addToFavorites(target!);
  ...
  const deleteOrArchiveForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await userServiceForUserCreatedIssuesImpl.deleteOrArchive(target!);
  ...
  const removeFromFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await userServiceForUserCreatedIssuesImpl.removeFromFavorites(target!);
  ...
  const activateForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await userServiceForUserCreatedIssuesImpl.activate(target!);
  ...
  const closeVoteForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await userServiceForUserCreatedIssuesImpl.closeVote(target!);
  ...
  return (
    <div
      id="User/(esm/_ITVmEFqnEe6_67aMO2jOsw)/AccessTablePageDefinition"
      data-page-name="service::User::userCreatedIssues::Access::Table::Page"
    >
      <Suspense>
