...
export const convertServiceUserUserCreatedIssuesAccessViewPagePayload = (
...
// XMIID: User/(esm/_ITVmEFqnEe6_67aMO2jOsw)/AccessViewPageDefinition
// Name: service::User::userCreatedIssues::Access::View::Page
export default function ServiceUserUserCreatedIssuesAccessViewPage() {
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] = convertServiceUserUserCreatedIssuesAccessViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const serviceIssueIssue_View_EditRefresh = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await userServiceForUserCreatedIssuesImpl.refresh(
  ...
  const serviceIssueIssue_View_EditUpdate = async () => {
    ...
    try {
      const res = await userServiceForUserCreatedIssuesImpl.update(payloadDiff.current);
  ...
  const serviceIssueIssue_View_EditActionsPageActionButtonsActivate = async () => {
    try {
      setIsLoading(true);
      await userServiceForUserCreatedIssuesImpl.activate(data);
  ...
  const serviceIssueIssue_View_EditActionsPageActionButtonsCloseVote = async () => {
    try {
      setIsLoading(true);
      await userServiceForUserCreatedIssuesImpl.closeVote(data);
  ...
  const serviceIssueIssue_View_EditActionsPageActionButtonsDeleteOrArchive = async () => {
    try {
      setIsLoading(true);
      await userServiceForUserCreatedIssuesImpl.deleteOrArchive(data);
  ...
  const serviceIssueIssue_View_EditActionsPageActionButtonsRemoveFromFavorites = async () => {
    try {
      setIsLoading(true);
      await userServiceForUserCreatedIssuesImpl.removeFromFavorites(data);
  ...
  const serviceIssueIssue_View_EditActionsPageActionButtonsAddToFavorites = async () => {
    try {
      setIsLoading(true);
      await userServiceForUserCreatedIssuesImpl.addToFavorites(data);
  ...
  const serviceIssueIssue_View_EditIssueOwnerAutocomplete = async (
    ...
    try {
      return userServiceForUserCreatedIssuesImpl.getRangeForOwner(data, queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const serviceIssueIssue_View_EditIssueIssueTypeAutocomplete = async (
    ...
    try {
      return userServiceForUserCreatedIssuesImpl.getRangeForIssueType(data, queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
