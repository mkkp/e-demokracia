...
export const SERVICE_USER_ISSUES_ACCESS_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_ISSUES_ACCESS_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceUserIssuesAccessViewPagePayload = (attributeName: keyof ServiceIssue, value: any): any => {
...
export default function ServiceUserIssuesAccessViewPage() {
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] = convertServiceUserIssuesAccessViewPagePayload(attributeName, value);
      setData((prevData) => ({
        ...prevData,
        [attributeName]: value,
      }));
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_ACCESS_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const commentsVoteDownForCommentAction = async (target?: ServiceCommentStored) => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.voteDownForComments(target!);
  ...
  const commentsVoteUpForCommentAction = async (target?: ServiceCommentStored) => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.voteUpForComments(target!);
  ...
  const consVoteDownForConAction = async (target?: ServiceConStored) => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.voteDownForCons(target!);
  ...
  const consVoteUpForConAction = async (target?: ServiceConStored) => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.voteUpForCons(target!);
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.closeVote(data);
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await userServiceForIssuesImpl.getRangeForIssueType(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await userServiceForIssuesImpl.getRangeForOwner(cleanUpPayload(data), queryCustomizer);
      return result;
    } catch (error: any) {
      handleError(error);
      return Promise.resolve([]);
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await userServiceForIssuesImpl.getRangeForCity(cleanUpPayload(data), queryCustomizer);
      return result;
    } catch (error: any) {
      handleError(error);
      return Promise.resolve([]);
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await userServiceForIssuesImpl.getRangeForCounty(cleanUpPayload(data), queryCustomizer);
      return result;
    } catch (error: any) {
      handleError(error);
      return Promise.resolve([]);
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await userServiceForIssuesImpl.getRangeForDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await userServiceForIssuesImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await userServiceForIssuesImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await userServiceForIssuesImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const prosVoteDownForProAction = async (target?: ServiceProStored) => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.voteDownForPros(target!);
  ...
  const prosVoteUpForProAction = async (target?: ServiceProStored) => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.voteUpForPros(target!);
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await userServiceForIssuesImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await userServiceForIssuesImpl.update(payloadDiff.current);
      if (res) {
    ...
    <ServiceIssueIssue_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          ...
          data-page-name="service::User::issues::AccessViewPage"
        />
