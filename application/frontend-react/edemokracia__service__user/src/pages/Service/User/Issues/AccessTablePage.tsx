...
export const SERVICE_USER_ISSUES_ACCESS_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_ISSUES_ACCESS_TABLE_PAGE_ACTIONS_HOOK';
...
// Name: service::User::issues::AccessTablePage
export default function ServiceUserIssuesAccessTablePage() {
  // Services
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_TableActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_ISSUES_ACCESS_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const activateForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.activate(target!);
  ...
  const addToFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.addToFavorites(target!);
  ...
  const closeVoteForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.closeVote(target!);
  ...
  const deleteOrArchiveForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.deleteOrArchive(target!);
  ...
  const removeFromFavoritesForIssueAction = async (target?: ServiceIssueStored) => {
    try {
      setIsLoading(true);
      await userServiceForIssuesImpl.removeFromFavorites(target!);
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      return userServiceForIssuesImpl.list(undefined, queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const openPageAction = async (target: ServiceIssue | ServiceIssueStored, isDraft?: boolean) => {
    if (isDraft && (!target || !(target as ServiceIssueStored).__signedIdentifier)) {
      ...
      navigate(routeToServiceUserIssuesAccessViewPage((target as ServiceIssueStored)!.__signedIdentifier));
    }
  };
    ...
    <ServiceIssueIssue_TableViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          ...
          data-page-name="service::User::issues::AccessTablePage"
        />
