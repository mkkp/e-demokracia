...
export default function ServiceUserAdminIssuesAccessViewPage() {
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      return userServiceForAdminIssuesImpl.getRangeForIssueType(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      return userServiceForAdminIssuesImpl.getRangeForOwner(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      return userServiceForAdminIssuesImpl.getRangeForCity(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      return userServiceForAdminIssuesImpl.getRangeForCounty(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      return userServiceForAdminIssuesImpl.getRangeForDistrict(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const refreshAction = async (queryCustomizer: ServiceIssueQueryCustomizer): Promise<ServiceIssueStored> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await userServiceForAdminIssuesImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const res = await userServiceForAdminIssuesImpl.update(payloadDiff.current);
      if (res) {
