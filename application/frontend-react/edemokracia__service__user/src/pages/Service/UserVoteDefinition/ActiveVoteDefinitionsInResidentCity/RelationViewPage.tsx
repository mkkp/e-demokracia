...
export const SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_RESIDENT_CITY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_RESIDENT_CITY_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export default function ServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCityRelationViewPage() {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentCityImpl.refresh(
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentCityImpl.update(
        payloadDiff.current,
      );
      if (res) {
  ...
  const issuePreFetchAction = async (): Promise<JudoRestResponse<ServiceIssueStored>> => {
    return serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentCityImpl.getIssue(
      { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
      {
        _mask: '{}',
      },
