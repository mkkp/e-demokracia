...
import { useServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCityAddSelectorPage } from '~/dialogs/Service/UserVoteDefinition/ActiveVoteDefinitionsInResidentCity/AddSelectorPage';
...
// Name: service::UserVoteDefinition::activeVoteDefinitionsInResidentCity::RelationTablePage
export default function ServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCityRelationTablePage() {
  ...
  // Dialog hooks
  const openServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCityAddSelectorPage =
    useServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCityAddSelectorPage();
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } =
      await openServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCityAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentCityImpl.addActiveVoteDefinitionsInResidentCity(
  ...
  const removeAction = async (target?: ServiceVoteDefinitionStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentCityImpl.removeActiveVoteDefinitionsInResidentCity(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
  ...
  return (
    <div
      ...
      data-page-name="service::UserVoteDefinition::activeVoteDefinitionsInResidentCity::RelationTablePage"
    >
      <Suspense>
