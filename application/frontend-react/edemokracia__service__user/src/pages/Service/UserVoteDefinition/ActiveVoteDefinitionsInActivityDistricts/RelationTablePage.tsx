...
import { useServiceUserVoteDefinitionActiveVoteDefinitionsInActivityDistrictsAddSelectorPage } from '~/dialogs/Service/UserVoteDefinition/ActiveVoteDefinitionsInActivityDistricts/AddSelectorPage';
...
export const SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_ACTIVITY_DISTRICTS_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
...
// XMIID: User/(esm/_kXCGYF5BEe6vsex_cZNQbQ)/RelationFeatureTable
// Name: service::UserVoteDefinition::activeVoteDefinitionsInActivityDistricts::RelationTablePage
export default function ServiceUserVoteDefinitionActiveVoteDefinitionsInActivityDistrictsRelationTablePage() {
  ...
  const { service: customActionsHook } = useTrackService<ServiceVoteDefinitionVoteDefinition_TableActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_ACTIVITY_DISTRICTS_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  // Dialog hooks
  const openServiceUserVoteDefinitionActiveVoteDefinitionsInActivityDistrictsAddSelectorPage =
    useServiceUserVoteDefinitionActiveVoteDefinitionsInActivityDistrictsAddSelectorPage();
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } =
      await openServiceUserVoteDefinitionActiveVoteDefinitionsInActivityDistrictsAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInActivityDistrictsImpl.addActiveVoteDefinitionsInActivityDistricts(
  ...
  const removeAction = async (target?: ServiceVoteDefinitionStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInActivityDistrictsImpl.removeActiveVoteDefinitionsInActivityDistricts(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      return serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInActivityDistrictsImpl.list(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
        queryCustomizer,
      );
    } catch (error) {
  ...
  return (
    <div
      id="User/(esm/_kXCGYF5BEe6vsex_cZNQbQ)/RelationFeatureTable"
      data-page-name="service::UserVoteDefinition::activeVoteDefinitionsInActivityDistricts::RelationTablePage"
    >
      <Suspense>
