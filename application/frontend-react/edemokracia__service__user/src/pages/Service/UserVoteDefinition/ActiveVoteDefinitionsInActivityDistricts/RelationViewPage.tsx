...
export const SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_ACTIVITY_DISTRICTS_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_ACTIVITY_DISTRICTS_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceUserVoteDefinitionActiveVoteDefinitionsInActivityDistrictsRelationViewPagePayload = (
...
// XMIID: User/(esm/_kXCGYF5BEe6vsex_cZNQbQ)/RelationFeatureView
// Name: service::UserVoteDefinition::activeVoteDefinitionsInActivityDistricts::RelationViewPage
export default function ServiceUserVoteDefinitionActiveVoteDefinitionsInActivityDistrictsRelationViewPage() {
  ...
  const storeDiff: (attributeName: keyof ServiceVoteDefinition, value: any) => void = useCallback(
    (attributeName: keyof ServiceVoteDefinition, value: any) => {
      payloadDiff.current![attributeName] =
        convertServiceUserVoteDefinitionActiveVoteDefinitionsInActivityDistrictsRelationViewPagePayload(
          attributeName,
          value,
        );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceVoteDefinitionVoteDefinition_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_ACTIVITY_DISTRICTS_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInActivityDistrictsImpl.refresh(
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const res = await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInActivityDistrictsImpl.update(
        payloadDiff.current,
      );
      if (res) {
  ...
  const issuePreFetchAction = async (): Promise<ServiceIssueStored> => {
    return serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInActivityDistrictsImpl.getIssue(
      { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
      {
        _mask: '{}',
      },
  ...
  return (
    <ServiceVoteDefinitionVoteDefinition_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_kXCGYF5BEe6vsex_cZNQbQ)/RelationFeatureView"
          data-page-name="service::UserVoteDefinition::activeVoteDefinitionsInActivityDistricts::RelationViewPage"
        />
