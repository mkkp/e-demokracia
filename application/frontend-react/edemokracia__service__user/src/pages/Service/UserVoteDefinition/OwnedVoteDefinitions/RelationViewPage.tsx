...
export const SERVICE_USER_VOTE_DEFINITION_OWNED_VOTE_DEFINITIONS_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_VOTE_DEFINITION_OWNED_VOTE_DEFINITIONS_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
// Name: service::UserVoteDefinition::ownedVoteDefinitions::RelationViewPage
export default function ServiceUserVoteDefinitionOwnedVoteDefinitionsRelationViewPage() {
  ...
  const { service: customActionsHook } = useTrackService<ServiceVoteDefinitionVoteDefinition_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_VOTE_DEFINITION_OWNED_VOTE_DEFINITIONS_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceUserVoteDefinitionServiceForOwnedVoteDefinitionsImpl.refresh(
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceUserVoteDefinitionServiceForOwnedVoteDefinitionsImpl.update(
        payloadDiff.current,
      );
      if (res) {
  ...
  return (
    <ServiceVoteDefinitionVoteDefinition_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_7OZ8MF5AEe6vsex_cZNQbQ)/RelationFeatureView"
          data-page-name="service::UserVoteDefinition::ownedVoteDefinitions::RelationViewPage"
        />
