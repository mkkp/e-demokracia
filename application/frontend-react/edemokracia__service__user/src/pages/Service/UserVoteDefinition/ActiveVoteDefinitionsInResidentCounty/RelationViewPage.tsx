...
export const SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCountyRelationViewPagePayload = (
...
// XMIID: User/(esm/_yd4FQF5BEe6vsex_cZNQbQ)/RelationFeatureView
// Name: service::UserVoteDefinition::activeVoteDefinitionsInResidentCounty::RelationViewPage
export default function ServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCountyRelationViewPage() {
  ...
  const storeDiff: (attributeName: keyof ServiceVoteDefinition, value: any) => void = useCallback(
    (attributeName: keyof ServiceVoteDefinition, value: any) => {
      payloadDiff.current![attributeName] =
        convertServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCountyRelationViewPagePayload(
          attributeName,
          value,
        );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceVoteDefinitionVoteDefinition_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentCountyImpl.refresh(
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentCountyImpl.update(
        payloadDiff.current,
      );
      if (res) {
  ...
  const issuePreFetchAction = async (): Promise<JudoRestResponse<ServiceIssueStored>> => {
    return serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentCountyImpl.getIssue(
      { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
      {
        _mask: '{}',
      },
  ...
  return (
    <ServiceVoteDefinitionVoteDefinition_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_yd4FQF5BEe6vsex_cZNQbQ)/RelationFeatureView"
          data-page-name="service::UserVoteDefinition::activeVoteDefinitionsInResidentCounty::RelationViewPage"
        />
