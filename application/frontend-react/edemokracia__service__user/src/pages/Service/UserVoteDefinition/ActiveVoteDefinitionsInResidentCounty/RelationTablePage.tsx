...
import { useServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCountyAddSelectorPage } from '~/dialogs/Service/UserVoteDefinition/ActiveVoteDefinitionsInResidentCounty/AddSelectorPage';
...
// XMIID: User/(esm/_yd4FQF5BEe6vsex_cZNQbQ)/RelationFeatureTable
// Name: service::UserVoteDefinition::activeVoteDefinitionsInResidentCounty::RelationTablePage
export default function ServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCountyRelationTablePage() {
  ...
  const { service: customActionsHook } = useTrackService<ServiceVoteDefinitionVoteDefinition_TableActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_RESIDENT_COUNTY_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  // Dialog hooks
  const openServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCountyAddSelectorPage =
    useServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCountyAddSelectorPage();
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } =
      await openServiceUserVoteDefinitionActiveVoteDefinitionsInResidentCountyAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentCountyImpl.addActiveVoteDefinitionsInResidentCounty(
  ...
  const removeAction = async (target?: ServiceVoteDefinitionStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentCountyImpl.removeActiveVoteDefinitionsInResidentCounty(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      return serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentCountyImpl.list(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
        queryCustomizer,
      );
    } catch (error) {
