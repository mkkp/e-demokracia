...
import { useServiceUserVoteDefinitionActiveVoteDefinitionsInActivityCitiesAddSelectorPage } from '~/dialogs/Service/UserVoteDefinition/ActiveVoteDefinitionsInActivityCities/AddSelectorPage';
...
export const SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_ACTIVITY_CITIES_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
...
// XMIID: User/(esm/_b8NPIF5BEe6vsex_cZNQbQ)/RelationFeatureTable
// Name: service::UserVoteDefinition::activeVoteDefinitionsInActivityCities::RelationTablePage
export default function ServiceUserVoteDefinitionActiveVoteDefinitionsInActivityCitiesRelationTablePage() {
  ...
  const { service: customActionsHook } = useTrackService<ServiceVoteDefinitionVoteDefinition_TableActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_ACTIVITY_CITIES_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  // Dialog hooks
  const openServiceUserVoteDefinitionActiveVoteDefinitionsInActivityCitiesAddSelectorPage =
    useServiceUserVoteDefinitionActiveVoteDefinitionsInActivityCitiesAddSelectorPage();
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } =
      await openServiceUserVoteDefinitionActiveVoteDefinitionsInActivityCitiesAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInActivityCitiesImpl.addActiveVoteDefinitionsInActivityCities(
  ...
  const removeAction = async (target?: ServiceVoteDefinitionStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInActivityCitiesImpl.removeActiveVoteDefinitionsInActivityCities(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      return serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInActivityCitiesImpl.list(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
        queryCustomizer,
      );
    } catch (error) {
  ...
  const openPageAction = async (target?: ServiceVoteDefinitionStored) => {
    ...
    navigate(
      routeToServiceUserVoteDefinitionActiveVoteDefinitionsInActivityCitiesRelationViewPage(target!.__signedIdentifier),
    );
  };
  ...
  return (
    <div
      id="User/(esm/_b8NPIF5BEe6vsex_cZNQbQ)/RelationFeatureTable"
      data-page-name="service::UserVoteDefinition::activeVoteDefinitionsInActivityCities::RelationTablePage"
    >
      <Suspense>
