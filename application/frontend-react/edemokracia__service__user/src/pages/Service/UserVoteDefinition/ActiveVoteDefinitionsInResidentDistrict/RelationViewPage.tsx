...
export const SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_RESIDENT_DISTRICT_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_RESIDENT_DISTRICT_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceUserVoteDefinitionActiveVoteDefinitionsInResidentDistrictRelationViewPagePayload = (
...
// XMIID: User/(esm/_omfCAF5EEe6vsex_cZNQbQ)/RelationFeatureView
// Name: service::UserVoteDefinition::activeVoteDefinitionsInResidentDistrict::RelationViewPage
export default function ServiceUserVoteDefinitionActiveVoteDefinitionsInResidentDistrictRelationViewPage() {
  ...
  const storeDiff: (attributeName: keyof ServiceVoteDefinition, value: any) => void = useCallback(
    (attributeName: keyof ServiceVoteDefinition, value: any) => {
      payloadDiff.current![attributeName] =
        convertServiceUserVoteDefinitionActiveVoteDefinitionsInResidentDistrictRelationViewPagePayload(
          attributeName,
          value,
        );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceVoteDefinitionVoteDefinition_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_IN_RESIDENT_DISTRICT_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentDistrictImpl.refresh(
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentDistrictImpl.update(
        payloadDiff.current,
      );
      if (res) {
  ...
  const issuePreFetchAction = async (): Promise<JudoRestResponse<ServiceIssueStored>> => {
    return serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInResidentDistrictImpl.getIssue(
      { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
      {
        _mask: '{}',
      },
  ...
  return (
    <ServiceVoteDefinitionVoteDefinition_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_omfCAF5EEe6vsex_cZNQbQ)/RelationFeatureView"
          data-page-name="service::UserVoteDefinition::activeVoteDefinitionsInResidentDistrict::RelationViewPage"
        />
