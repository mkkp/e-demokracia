...
import { useServiceUserVoteDefinitionActiveVoteDefinitionsInActivityCountiesAddSelectorPage } from '~/dialogs/Service/UserVoteDefinition/ActiveVoteDefinitionsInActivityCounties/AddSelectorPage';
...
export default function ServiceUserVoteDefinitionActiveVoteDefinitionsInActivityCountiesRelationTablePage() {
  ...
  // Dialog hooks
  const openServiceUserVoteDefinitionActiveVoteDefinitionsInActivityCountiesAddSelectorPage =
    useServiceUserVoteDefinitionActiveVoteDefinitionsInActivityCountiesAddSelectorPage();
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } =
      await openServiceUserVoteDefinitionActiveVoteDefinitionsInActivityCountiesAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInActivityCountiesImpl.addActiveVoteDefinitionsInActivityCounties(
  ...
  const removeAction = async (target?: ServiceVoteDefinitionStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsInActivityCountiesImpl.removeActiveVoteDefinitionsInActivityCounties(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
