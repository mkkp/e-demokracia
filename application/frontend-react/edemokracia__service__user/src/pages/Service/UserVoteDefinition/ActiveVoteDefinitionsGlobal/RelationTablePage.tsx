...
import { useServiceUserVoteDefinitionActiveVoteDefinitionsGlobalAddSelectorPage } from '~/dialogs/Service/UserVoteDefinition/ActiveVoteDefinitionsGlobal/AddSelectorPage';
...
export const SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_GLOBAL_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
...
// XMIID: User/(esm/_HCMTUF5BEe6vsex_cZNQbQ)/RelationFeatureTable
// Name: service::UserVoteDefinition::activeVoteDefinitionsGlobal::RelationTablePage
export default function ServiceUserVoteDefinitionActiveVoteDefinitionsGlobalRelationTablePage() {
  ...
  const { service: customActionsHook } = useTrackService<ServiceVoteDefinitionVoteDefinition_TableActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_VOTE_DEFINITION_ACTIVE_VOTE_DEFINITIONS_GLOBAL_RELATION_TABLE_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  // Dialog hooks
  const openServiceUserVoteDefinitionActiveVoteDefinitionsGlobalAddSelectorPage =
    useServiceUserVoteDefinitionActiveVoteDefinitionsGlobalAddSelectorPage();
  ...
  const openAddSelectorAction = async () => {
    const { result, data: returnedData } =
      await openServiceUserVoteDefinitionActiveVoteDefinitionsGlobalAddSelectorPage(
    ...
    if (result === 'submit') {
      if (Array.isArray(returnedData) && returnedData.length) {
        try {
          setIsLoading(true);
          await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsGlobalImpl.addActiveVoteDefinitionsGlobal(
  ...
  const removeAction = async (target?: ServiceVoteDefinitionStored, silentMode?: boolean) => {
    if (target) {
      try {
        ...
        await serviceUserVoteDefinitionServiceForActiveVoteDefinitionsGlobalImpl.removeActiveVoteDefinitionsGlobal(
          { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
          [target!],
        );
        if (!silentMode) {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      return serviceUserVoteDefinitionServiceForActiveVoteDefinitionsGlobalImpl.list(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
        queryCustomizer,
      );
    } catch (error) {
  ...
  const openPageAction = async (target?: ServiceVoteDefinitionStored) => {
    ...
    navigate(routeToServiceUserVoteDefinitionActiveVoteDefinitionsGlobalRelationViewPage(target!.__signedIdentifier));
  };
  ...
  return (
    <div
      id="User/(esm/_HCMTUF5BEe6vsex_cZNQbQ)/RelationFeatureTable"
      data-page-name="service::UserVoteDefinition::activeVoteDefinitionsGlobal::RelationTablePage"
    >
      <Suspense>
