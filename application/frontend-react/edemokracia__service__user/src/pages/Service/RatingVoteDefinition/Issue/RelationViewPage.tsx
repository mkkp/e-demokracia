...
import { ServiceRatingVoteDefinitionServiceForIssueImpl } from '~/services/data-axios/ServiceRatingVoteDefinitionServiceForIssueImpl';
...
export const SERVICE_RATING_VOTE_DEFINITION_ISSUE_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_RATING_VOTE_DEFINITION_ISSUE_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceRatingVoteDefinitionIssueRelationViewPagePayload = (
...
// XMIID: User/(esm/_FUKWEFohEe6_67aMO2jOsw)/RelationFeatureView
// Name: service::RatingVoteDefinition::issue::RelationViewPage
export default function ServiceRatingVoteDefinitionIssueRelationViewPage() {
  // Router params section
  const { signedIdentifier } = useParams();

  // Services
  const serviceRatingVoteDefinitionServiceForIssueImpl = useMemo(
  ...
  const storeDiff: (attributeName: keyof ServiceIssue, value: any) => void = useCallback(
    (attributeName: keyof ServiceIssue, value: any) => {
      payloadDiff.current![attributeName] = convertServiceRatingVoteDefinitionIssueRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceIssueIssue_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_RATING_VOTE_DEFINITION_ISSUE_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const activateForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceRatingVoteDefinitionServiceForIssueImpl.activate(data);
  ...
  const addToFavoritesForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceRatingVoteDefinitionServiceForIssueImpl.addToFavorites(data);
  ...
  const closeVoteForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceRatingVoteDefinitionServiceForIssueImpl.closeVote(data);
  ...
  const issueTypeAutocompleteRangeAction = async (
    ...
    try {
      return serviceRatingVoteDefinitionServiceForIssueImpl.getRangeForIssueType(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      return serviceRatingVoteDefinitionServiceForIssueImpl.getRangeForOwner(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const cityAutocompleteRangeAction = async (
    ...
    try {
      return serviceRatingVoteDefinitionServiceForIssueImpl.getRangeForCity(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const countyAutocompleteRangeAction = async (
    ...
    try {
      return serviceRatingVoteDefinitionServiceForIssueImpl.getRangeForCounty(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const districtAutocompleteRangeAction = async (
    ...
    try {
      return serviceRatingVoteDefinitionServiceForIssueImpl.getRangeForDistrict(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const consDeleteAction = async (target: ServiceConStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceRatingVoteDefinitionServiceForIssueImpl.deleteCons(target);
        if (!silentMode) {
  ...
  const prosDeleteAction = async (target: ServiceProStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceRatingVoteDefinitionServiceForIssueImpl.deletePros(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const attachmentsDeleteAction = async (target: ServiceIssueAttachmentStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceRatingVoteDefinitionServiceForIssueImpl.deleteAttachments(target);
        if (!silentMode) {
          showSuccessSnack(t('judo.action.delete.success', { defaultValue: 'Delete successful' }));
          refreshAction(getPageQueryCustomizer());
        }
  ...
  const refreshAction = async (queryCustomizer: ServiceIssueQueryCustomizer): Promise<ServiceIssueStored> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceRatingVoteDefinitionServiceForIssueImpl.refresh(
        { __signedIdentifier: signedIdentifier } as JudoIdentifiable<any>,
  ...
  return (
    <ServiceIssueIssue_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_FUKWEFohEe6_67aMO2jOsw)/RelationFeatureView"
          data-page-name="service::RatingVoteDefinition::issue::RelationViewPage"
        />
