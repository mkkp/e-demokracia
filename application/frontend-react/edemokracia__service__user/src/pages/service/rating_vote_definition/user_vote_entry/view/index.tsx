...
// Page name: edemokracia::service::RatingVoteDefinition.userVoteEntry#View
...
import { serviceRatingVoteEntryServiceForClassImpl } from '~/generated/data-axios';
import {} from './actions';

import { OwnerLink } from './components/OwnerLink';

export interface ServiceUserViewProps {
  entry: ServiceRatingVoteEntryStored;
...
export type ServiceUserViewPostRefreshAction = (
  ...
  storeDiff: (attributeName: keyof ServiceRatingVoteEntryStored, value: any) => void,
...
/**
 * Name: edemokracia::service::RatingVoteDefinition.userVoteEntry#View
 * Is Access: false
 * Type: View
...
export default function ServiceUserView(props: ServiceUserViewProps) {
  ...
  const [data, setData] = useState<ServiceRatingVoteEntryStored>(
    entry ? { ...entry } : ({} as unknown as ServiceRatingVoteEntryStored),
  );
  const [payloadDiff, setPayloadDiff] = useState<Record<keyof ServiceRatingVoteEntryStored, any>>(
    {} as unknown as Record<keyof ServiceRatingVoteEntryStored, any>,
  );
  const [editMode, setEditMode] = useState<boolean>(false);
  const storeDiff: (attributeName: keyof ServiceRatingVoteEntryStored, value: any) => void = useCallback(
    (attributeName: keyof ServiceRatingVoteEntryStored, value: any) => {
  ...
  const title: string = t('service.RatingVoteEntryView', { defaultValue: 'RatingVoteEntry View / Edit' });
  ...
  async function fetchData() {
    ...
    try {
      const res = await serviceRatingVoteEntryServiceForClassImpl.refresh(
        { __signedIdentifier: entry.__signedIdentifier } as JudoIdentifiable<ServiceRatingVoteEntry>,
        ...
        <Grid container spacing={2} direction="column" alignItems="stretch" justifyContent="flex-start">
          <Grid item xs={12} sm={12}>
            <DateTimePicker
              ...
              slotProps={{
                textField: {
                  id: 'DateTimeInputedemokraciaServiceUserEdemokraciaServiceRatingVoteDefinitionUserVoteEntryViewDefaultRatingVoteEntryViewEditCreated',
                  required: true,
              ...
              label={t('service.RatingVoteEntryView.created', { defaultValue: 'Created' }) as string}
          ...
          <Grid item xs={12} sm={12}>
            <NumericInput
              required={true}
              name="value"
              id="NumericInputedemokraciaServiceUserEdemokraciaServiceRatingVoteDefinitionUserVoteEntryViewDefaultRatingVoteEntryViewEditValue"
              label={t('service.RatingVoteEntryView.value', { defaultValue: 'Value' }) as string}
