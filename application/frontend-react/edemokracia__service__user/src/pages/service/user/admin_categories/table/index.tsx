...
// Page name: edemokracia::service::User.adminCategories#Table
...
/**
 * Name: edemokracia::service::User.adminCategories#Table
 * Is Access: true
 * Is Dashboard: false
 * Type: Table
 **/
export default function ServiceUserAdminCategoriesTable() {
  const tableRef = useRef<RefreshableTable>(null);
  const { t } = useTranslation();
  const { locale: l10nLocale } = useL10N();
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const title: string = t('service.IssueCategoryTable', { defaultValue: 'IssueCategory Table' });
      ...
      <PageContainerTransition>
        <Box sx={mainContainerPadding}>
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Card>
                <CardContent id="PageDefinitionedemokraciaServiceUserEdemokraciaServiceUserAdminCategoriesTable-data-grid">
