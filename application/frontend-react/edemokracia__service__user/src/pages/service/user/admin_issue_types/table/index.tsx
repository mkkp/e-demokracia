...
// Page name: edemokracia::service::User.adminIssueTypes#Table
...
/**
 * Name: edemokracia::service::User.adminIssueTypes#Table
 * Is Access: true
 * Is Dashboard: false
 * Type: Table
 **/
export default function ServiceUserAdminIssueTypesTable() {
  const tableRef = useRef<RefreshableTable>(null);
  const { t } = useTranslation();
  const { locale: l10nLocale } = useL10N();
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const title: string = t('service.IssueTypeTable', { defaultValue: 'IssueType Table' });
      ...
      <PageContainerTransition>
        <Box sx={mainContainerPadding}>
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Card>
                <CardContent id="PageDefinitionedemokraciaServiceUserEdemokraciaServiceUserAdminIssueTypesTable-data-grid">
