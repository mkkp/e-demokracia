...
// Page name: edemokracia::service::SelectAnswerVoteDefinition.userVoteEntry#View
...
import { serviceSelectAnswerVoteEntryServiceForClassImpl } from '~/generated/data-axios';
import {} from './actions';

import { OwnerLink } from './components/OwnerLink';

export interface ServiceUserViewProps {
  entry: ServiceSelectAnswerVoteEntryStored;
...
export type ServiceUserViewPostRefreshAction = (
  ...
  storeDiff: (attributeName: keyof ServiceSelectAnswerVoteEntryStored, value: any) => void,
...
/**
 * Name: edemokracia::service::SelectAnswerVoteDefinition.userVoteEntry#View
 * Is Access: false
 * Type: View
...
export default function ServiceUserView(props: ServiceUserViewProps) {
  ...
  const [data, setData] = useState<ServiceSelectAnswerVoteEntryStored>(
    entry ? { ...entry } : ({} as unknown as ServiceSelectAnswerVoteEntryStored),
  );
  const [payloadDiff, setPayloadDiff] = useState<Record<keyof ServiceSelectAnswerVoteEntryStored, any>>(
    {} as unknown as Record<keyof ServiceSelectAnswerVoteEntryStored, any>,
  );
  const [editMode, setEditMode] = useState<boolean>(false);
  const storeDiff: (attributeName: keyof ServiceSelectAnswerVoteEntryStored, value: any) => void = useCallback(
    (attributeName: keyof ServiceSelectAnswerVoteEntryStored, value: any) => {
  ...
  const title: string = t('service.SelectAnswerVoteEntryView', { defaultValue: 'SelectAnswerVoteEntry View / Edit' });
  ...
  async function fetchData() {
    ...
    try {
      const res = await serviceSelectAnswerVoteEntryServiceForClassImpl.refresh(
        { __signedIdentifier: entry.__signedIdentifier } as JudoIdentifiable<ServiceSelectAnswerVoteEntry>,
        ...
        <Grid container spacing={2} direction="column" alignItems="stretch" justifyContent="flex-start">
          <Grid item xs={12} sm={12}>
            <DateTimePicker
              ...
              slotProps={{
                textField: {
                  id: 'DateTimeInputedemokraciaServiceUserEdemokraciaServiceSelectAnswerVoteDefinitionUserVoteEntryViewDefaultSelectAnswerVoteEntryViewEditCreated',
                  required: true,
              ...
              label={t('service.SelectAnswerVoteEntryView.created', { defaultValue: 'Created' }) as string}
          ...
          <Grid item xs={12} sm={12}>
            <TextField
              ...
              id="TextInputedemokraciaServiceUserEdemokraciaServiceSelectAnswerVoteDefinitionUserVoteEntryViewDefaultSelectAnswerVoteEntryViewEditValueRepresentation"
              label={
                t('service.SelectAnswerVoteEntryView.valueRepresentation', {
