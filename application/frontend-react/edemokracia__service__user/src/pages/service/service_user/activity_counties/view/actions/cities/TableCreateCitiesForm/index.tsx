...
import { serviceCountyServiceForCitiesImpl, serviceCityServiceForClassImpl } from '~/generated/data-axios';

export interface TableCreateCitiesFormProps {
  successCallback: (result: ServiceCityStored, open?: boolean) => void;
  cancel: () => void;
...
export default function TableCreateCitiesForm({ successCallback, cancel, owner }: TableCreateCitiesFormProps) {
  ...
  const title: string = t('service.CityForm', { defaultValue: 'City Form' });
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await serviceCityServiceForClassImpl.getTemplate();
      setData((prevData) => ({ ...prevData, ...res }));
      setPayloadDiff({
        ...res,
      } as Record<keyof ServiceCity, any>);
    } catch (error) {
  ...
  const submit = async () => {
    ...
    try {
      const res = await serviceCountyServiceForCitiesImpl.createCities(owner, payloadDiff);
  ...
  return (
    <>
      <DialogTitle>
        {title}
        <IconButton
          id="CreateActionedemokraciaServiceUserEdemokraciaServiceServiceUserActivityCountiesViewEdemokraciaServiceUserEdemokraciaServiceCountyCitiesTableCreate-dialog-close"
          aria-label="close"
          onClick={() => cancel()}
          sx={{
            position: 'absolute',
        ...
        <Grid container spacing={2} direction="column" alignItems="stretch" justifyContent="flex-start">
          <Grid item xs={12} sm={12}>
            <TextField
              required={true}
              name="name"
              id="TextInputedemokraciaServiceUserEdemokraciaServiceCountyCitiesCreateDefaultCityFormName"
              autoFocus
              label={t('service.CityForm.name', { defaultValue: 'City name' }) as string}
              value={data.name ?? ''}
              className={clsx({
      ...
      <DialogActions>
        <Button
          id="CreateActionedemokraciaServiceUserEdemokraciaServiceServiceUserActivityCountiesViewEdemokraciaServiceUserEdemokraciaServiceCountyCitiesTableCreate-action-form-action-cancel"
          variant="text"
          onClick={() => cancel()}
          disabled={isLoading}
        >
        ...
        <ButtonGroup size="small" ref={anchorRef} aria-label="split button" disabled={isLoading}>
          <LoadingButton
            ...
            id="CreateActionedemokraciaServiceUserEdemokraciaServiceServiceUserActivityCountiesViewEdemokraciaServiceUserEdemokraciaServiceCountyCitiesTableCreate-action-form-action-create"
            variant="contained"
            onClick={async () => {
                  ...
                  <MenuList id="split-button-menu" autoFocusItem>
                    <MenuItem
                      ...
                      onClick={async (event: any) => {
                        const result: ServiceCityStored | undefined = await submit();
