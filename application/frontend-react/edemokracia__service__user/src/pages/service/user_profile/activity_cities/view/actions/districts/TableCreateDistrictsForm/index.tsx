...
export interface TableCreateDistrictsFormProps {
  successCallback: (result: ServiceDistrictStored, open?: boolean) => void;
  cancel: () => void;
...
export default function TableCreateDistrictsForm({ successCallback, cancel, owner }: TableCreateDistrictsFormProps) {
  ...
  const title: string = t('service.DistrictForm', { defaultValue: 'District Form' });
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await serviceDistrictServiceForClassImpl.getTemplate();
      setData((prevData) => ({ ...prevData, ...res }));
      setPayloadDiff({
        ...res,
      } as Record<keyof ServiceDistrict, any>);
    } catch (error) {
  ...
  const submit = async () => {
    ...
    try {
      const res = await serviceCityServiceForDistrictsImpl.createDistricts(owner, payloadDiff);
  ...
  return (
    <>
      <DialogTitle>
        {title}
        <IconButton
          id="CreateActionedemokraciaServiceUserEdemokraciaServiceUserProfileActivityCitiesViewEdemokraciaServiceUserEdemokraciaServiceCityDistrictsTableCreate-dialog-close"
          aria-label="close"
          onClick={() => cancel()}
          sx={{
            position: 'absolute',
        ...
        <Grid container spacing={2} direction="column" alignItems="stretch" justifyContent="flex-start">
          <Grid item xs={12} sm={12}>
            <TextField
              required={true}
              name="name"
              id="TextInputedemokraciaServiceUserEdemokraciaServiceCityDistrictsCreateDefaultDistrictFormName"
              autoFocus
              label={t('service.DistrictForm.name', { defaultValue: 'Name' }) as string}
              value={data.name ?? ''}
              className={clsx({
      ...
      <DialogActions>
        <Button
          id="CreateActionedemokraciaServiceUserEdemokraciaServiceUserProfileActivityCitiesViewEdemokraciaServiceUserEdemokraciaServiceCityDistrictsTableCreate-action-form-action-cancel"
          variant="text"
          onClick={() => cancel()}
          disabled={isLoading}
        >
        ...
        <ButtonGroup size="small" ref={anchorRef} aria-label="split button" disabled={isLoading}>
          <LoadingButton
            ...
            id="CreateActionedemokraciaServiceUserEdemokraciaServiceUserProfileActivityCitiesViewEdemokraciaServiceUserEdemokraciaServiceCityDistrictsTableCreate-action-form-action-create"
            variant="contained"
            onClick={async () => {
                  ...
                  <MenuList id="split-button-menu" autoFocusItem>
                    <MenuItem
                      ...
                      onClick={async (event: any) => {
                        const result: ServiceDistrictStored | undefined = await submit();
