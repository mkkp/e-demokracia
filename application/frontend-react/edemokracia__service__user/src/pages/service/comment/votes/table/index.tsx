...
// Page name: edemokracia::service::Comment.votes#Table
...
/**
 * Name: edemokracia::service::Comment.votes#Table
 * Is Access: false
 * Type: Table
 **/
export default function ServiceCommentVotesTable() {
  const tableRef = useRef<RefreshableTable>(null);
  const { t } = useTranslation();
  const { signedIdentifier } = useParams();
  const { locale: l10nLocale } = useL10N();
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const title: string = t('service.SimpleVoteTable', { defaultValue: 'SimpleVote Table' });
      ...
      <PageContainerTransition>
        <Box sx={mainContainerPadding}>
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Card>
                <CardContent id="PageDefinitionedemokraciaServiceUserEdemokraciaServiceCommentVotesTable-data-grid">
