...
// Page name: edemokracia::service::YesNoAbstainVoteDefinition.userVoteEntry#View
...
import { serviceYesNoAbstainVoteEntryServiceForClassImpl } from '~/generated/data-axios';
import {} from './actions';

import { UserLink } from './components/UserLink';

export interface ServiceUserViewProps {
  entry: ServiceYesNoAbstainVoteEntryStored;
...
export type ServiceUserViewPostRefreshAction = (
  ...
  storeDiff: (attributeName: keyof ServiceYesNoAbstainVoteEntryStored, value: any) => void,
...
/**
 * Name: edemokracia::service::YesNoAbstainVoteDefinition.userVoteEntry#View
 * Is Access: false
 * Type: View
...
export default function ServiceUserView(props: ServiceUserViewProps) {
  ...
  const [data, setData] = useState<ServiceYesNoAbstainVoteEntryStored>(
    entry ? { ...entry } : ({} as unknown as ServiceYesNoAbstainVoteEntryStored),
  );
  const [payloadDiff, setPayloadDiff] = useState<Record<keyof ServiceYesNoAbstainVoteEntryStored, any>>(
    {} as unknown as Record<keyof ServiceYesNoAbstainVoteEntryStored, any>,
  );
  const [editMode, setEditMode] = useState<boolean>(false);
  const storeDiff: (attributeName: keyof ServiceYesNoAbstainVoteEntryStored, value: any) => void = useCallback(
    (attributeName: keyof ServiceYesNoAbstainVoteEntryStored, value: any) => {
  ...
  const title: string = t('service.YesNoAbstainVoteEntryView', { defaultValue: 'YesNoAbstainVoteEntry View / Edit' });
  ...
  async function fetchData() {
    ...
    try {
      const res = await serviceYesNoAbstainVoteEntryServiceForClassImpl.refresh(
        { __signedIdentifier: entry.__signedIdentifier } as JudoIdentifiable<ServiceYesNoAbstainVoteEntry>,
        ...
        <Grid container spacing={2} direction="column" alignItems="stretch" justifyContent="flex-start">
          <Grid item xs={12} sm={12}>
            <DateTimePicker
              ...
              slotProps={{
                textField: {
                  id: 'DateTimeInputedemokraciaServiceUserEdemokraciaServiceYesNoAbstainVoteDefinitionUserVoteEntryViewDefaultYesNoAbstainVoteEntryViewEditCreated',
                  required: true,
              ...
              label={t('service.YesNoAbstainVoteEntryView.created', { defaultValue: 'Created' }) as string}
          ...
          <Grid item xs={12} sm={12}>
            <TextField
              required={true}
              name="value"
              id="EnumerationComboedemokraciaServiceUserEdemokraciaServiceYesNoAbstainVoteDefinitionUserVoteEntryViewDefaultYesNoAbstainVoteEntryViewEditValue"
              label={t('service.YesNoAbstainVoteEntryView.value', { defaultValue: 'Value' }) as string}
              value={data.value || ''}
              className={clsx({
                'JUDO-viewMode': !editMode,
                'JUDO-required': true,
              ...
              select
            >
              <MenuItem id="EnumerationMemberedemokraciaServiceUserEdemokraciaYesNoAbstainVoteValueYES" value={'YES'}>
                {t('enumerations.EdemokraciaYesNoAbstainVoteValue.YES', { defaultValue: 'YES' })}
              </MenuItem>
              <MenuItem id="EnumerationMemberedemokraciaServiceUserEdemokraciaYesNoAbstainVoteValueNO" value={'NO'}>
                {t('enumerations.EdemokraciaYesNoAbstainVoteValue.NO', { defaultValue: 'NO' })}
              </MenuItem>
              <MenuItem
                id="EnumerationMemberedemokraciaServiceUserEdemokraciaYesNoAbstainVoteValueABSTAIN"
                value={'ABSTAIN'}
              >
