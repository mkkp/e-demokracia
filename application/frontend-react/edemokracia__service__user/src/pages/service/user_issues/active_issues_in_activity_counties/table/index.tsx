...
// Page name: edemokracia::service::UserIssues.activeIssuesInActivityCounties#Table
...
/**
 * Name: edemokracia::service::UserIssues.activeIssuesInActivityCounties#Table
 * Is Access: false
 * Type: Table
 **/
export default function ServiceUserIssuesActiveIssuesInActivityCountiesTable() {
  const tableRef = useRef<RefreshableTable>(null);
  const { t } = useTranslation();
  const { signedIdentifier } = useParams();
  const { locale: l10nLocale } = useL10N();
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const title: string = t('service.IssueTable', { defaultValue: 'Issue Table' });
      ...
      <PageContainerTransition>
        <Box sx={mainContainerPadding}>
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Card>
                <CardContent id="PageDefinitionedemokraciaServiceUserEdemokraciaServiceUserIssuesActiveIssuesInActivityCountiesTable-data-grid">
                  <Issue_TableTable
                    ref={tableRef}
