...
// Page name: edemokracia::service::YesNoVoteDefinition.userVoteEntry#View
...
import { serviceYesNoVoteEntryServiceForClassImpl } from '~/generated/data-axios';
import {} from './actions';
...
export type ServiceUserViewPostRefreshAction = (
  ...
  storeDiff: (attributeName: keyof ServiceYesNoVoteEntryStored, value: any) => void,
...
/**
 * Name: edemokracia::service::YesNoVoteDefinition.userVoteEntry#View
 * Is Access: false
 * Type: View
...
export default function ServiceUserView(props: ServiceUserViewProps) {
  ...
  const [data, setData] = useState<ServiceYesNoVoteEntryStored>(
    entry ? { ...entry } : ({} as unknown as ServiceYesNoVoteEntryStored),
  );
  const [payloadDiff, setPayloadDiff] = useState<Record<keyof ServiceYesNoVoteEntryStored, any>>(
    {} as unknown as Record<keyof ServiceYesNoVoteEntryStored, any>,
  );
  const [editMode, setEditMode] = useState<boolean>(false);
  const storeDiff: (attributeName: keyof ServiceYesNoVoteEntryStored, value: any) => void = useCallback(
    (attributeName: keyof ServiceYesNoVoteEntryStored, value: any) => {
  ...
  const title: string = t('service.YesNoVoteEntryView', { defaultValue: 'YesNoVoteEntry View / Edit' });
  ...
  async function fetchData() {
    ...
    try {
      const res = await serviceYesNoVoteEntryServiceForClassImpl.refresh(
        { __signedIdentifier: entry.__signedIdentifier } as JudoIdentifiable<ServiceYesNoVoteEntry>,
        ...
        <Grid container spacing={2} direction="column" alignItems="stretch" justifyContent="flex-start">
          <Grid item xs={12} sm={12}>
            <DateTimePicker
              ...
              slotProps={{
                textField: {
                  id: 'DateTimeInputedemokraciaServiceUserEdemokraciaServiceYesNoVoteDefinitionUserVoteEntryViewDefaultYesNoVoteEntryViewEditCreated',
                  required: true,
              ...
              label={t('service.YesNoVoteEntryView.created', { defaultValue: 'Created' }) as string}
          ...
          <Grid item xs={12} sm={12}>
            <TextField
              required={true}
              name="value"
              id="EnumerationComboedemokraciaServiceUserEdemokraciaServiceYesNoVoteDefinitionUserVoteEntryViewDefaultYesNoVoteEntryViewEditValue"
              label={t('service.YesNoVoteEntryView.value', { defaultValue: 'Value' }) as string}
              value={data.value || ''}
              className={clsx({
                'JUDO-viewMode': !editMode,
                'JUDO-required': true,
