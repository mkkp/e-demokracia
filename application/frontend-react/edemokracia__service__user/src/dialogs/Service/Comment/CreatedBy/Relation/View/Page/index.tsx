...
import { serviceCommentServiceForCreatedByImpl } from '~/services/data-axios';

export const useServiceCommentCreatedByRelationViewPage = (): ((
  ownerData: any,
  ...
  return (ownerData: any) =>
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceCommentCreatedByRelationViewPage
            ownerData={ownerData}
            onClose={() => {
              closeDialog();
              resolve({
...
export const convertServiceCommentCreatedByRelationViewPagePayload = (
...
export interface ServiceCommentCreatedByRelationViewPageProps {
  ownerData: any;
...
// Name: service::Comment::createdBy::Relation::View::Page
export default function ServiceCommentCreatedByRelationViewPage(props: ServiceCommentCreatedByRelationViewPageProps) {
  ...
  const storeDiff: (attributeName: keyof ServiceServiceUser, value: any) => void = useCallback(
    (attributeName: keyof ServiceServiceUser, value: any) => {
      payloadDiff.current![attributeName] = convertServiceCommentCreatedByRelationViewPagePayload(attributeName, value);
      setData((prevData) => ({
        ...prevData,
        [attributeName]: value,
      }));
  ...
  const serviceServiceUserServiceUser_View_EditRefresh = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceCommentServiceForCreatedByImpl.refresh(ownerData, pageQueryCustomizer);
  ...
  const serviceServiceUserServiceUser_View_EditAreasResidencyResidentCityAutocomplete = async (
    ...
    try {
      return serviceCommentServiceForCreatedByImpl.getRangeForResidentCity(data, queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const serviceServiceUserServiceUser_View_EditAreasResidencyResidentDistrictAutocomplete = async (
    ...
    try {
      return serviceCommentServiceForCreatedByImpl.getRangeForResidentDistrict(data, queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const serviceServiceUserServiceUser_View_EditAreasResidencyResidentCountyAutocomplete = async (
    ...
    try {
      return serviceCommentServiceForCreatedByImpl.getRangeForResidentCounty(data, queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
