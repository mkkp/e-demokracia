...
export default function ServiceRatingVoteDefinitionVoteEntriesRelationViewPage(
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceRatingVoteDefinitionServiceForVoteEntriesImpl.refresh(
        ownerData,
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      return serviceRatingVoteDefinitionServiceForVoteEntriesImpl.getRangeForOwner(
        cleanUpPayload(data),
        queryCustomizer,
      );
    } catch (error) {
