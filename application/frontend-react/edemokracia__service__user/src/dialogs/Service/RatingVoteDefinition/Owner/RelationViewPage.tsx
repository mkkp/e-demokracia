...
export const SERVICE_RATING_VOTE_DEFINITION_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_RATING_VOTE_DEFINITION_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceRatingVoteDefinitionOwnerRelationViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceServiceUser>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceRatingVoteDefinitionOwnerRelationViewPage
            ownerData={ownerData}
...
export const convertServiceRatingVoteDefinitionOwnerRelationViewPagePayload = (
...
export interface ServiceRatingVoteDefinitionOwnerRelationViewPageProps {
  ownerData: any;
...
// XMIID: User/(esm/_TzqgwHz4Ee6bP4FWw7fjQA)/RelationFeatureView
// Name: service::RatingVoteDefinition::owner::RelationViewPage
export default function ServiceRatingVoteDefinitionOwnerRelationViewPage(
  props: ServiceRatingVoteDefinitionOwnerRelationViewPageProps,
) {
  ...
  const storeDiff: (attributeName: keyof ServiceServiceUser, value: any) => void = useCallback(
    (attributeName: keyof ServiceServiceUser, value: any) => {
      payloadDiff.current![attributeName] = convertServiceRatingVoteDefinitionOwnerRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceServiceUserServiceUser_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_RATING_VOTE_DEFINITION_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceRatingVoteDefinitionServiceForOwnerImpl.refresh(
        ownerData,
  ...
  const residentCityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceRatingVoteDefinitionServiceForOwnerImpl.getRangeForResidentCity(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const residentCountyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceRatingVoteDefinitionServiceForOwnerImpl.getRangeForResidentCounty(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const residentDistrictAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceRatingVoteDefinitionServiceForOwnerImpl.getRangeForResidentDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  return (
    <ServiceServiceUserServiceUser_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_TzqgwHz4Ee6bP4FWw7fjQA)/RelationFeatureView"
          data-page-name="service::RatingVoteDefinition::owner::RelationViewPage"
        />
