...
export const SERVICE_YES_NO_ABSTAIN_VOTE_ENTRY_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_YES_NO_ABSTAIN_VOTE_ENTRY_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceYesNoAbstainVoteEntryOwnerRelationViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceServiceUser>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceYesNoAbstainVoteEntryOwnerRelationViewPage
            ownerData={ownerData}
...
export const convertServiceYesNoAbstainVoteEntryOwnerRelationViewPagePayload = (
...
export interface ServiceYesNoAbstainVoteEntryOwnerRelationViewPageProps {
  ownerData: any;
...
// XMIID: User/(esm/_jMSTIFsjEe6Mx9dH3yj5gQ)/RelationFeatureView
// Name: service::YesNoAbstainVoteEntry::owner::RelationViewPage
export default function ServiceYesNoAbstainVoteEntryOwnerRelationViewPage(
  props: ServiceYesNoAbstainVoteEntryOwnerRelationViewPageProps,
) {
  ...
  const storeDiff: (attributeName: keyof ServiceServiceUser, value: any) => void = useCallback(
    (attributeName: keyof ServiceServiceUser, value: any) => {
      payloadDiff.current![attributeName] = convertServiceYesNoAbstainVoteEntryOwnerRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceServiceUserServiceUser_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_YES_NO_ABSTAIN_VOTE_ENTRY_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const residentCityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoAbstainVoteEntryServiceForOwnerImpl.getRangeForResidentCity(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const residentCountyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoAbstainVoteEntryServiceForOwnerImpl.getRangeForResidentCounty(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const residentDistrictAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoAbstainVoteEntryServiceForOwnerImpl.getRangeForResidentDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceYesNoAbstainVoteEntryServiceForOwnerImpl.refresh(
        ownerData,
  ...
  return (
    <ServiceServiceUserServiceUser_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_jMSTIFsjEe6Mx9dH3yj5gQ)/RelationFeatureView"
          data-page-name="service::YesNoAbstainVoteEntry::owner::RelationViewPage"
        />
