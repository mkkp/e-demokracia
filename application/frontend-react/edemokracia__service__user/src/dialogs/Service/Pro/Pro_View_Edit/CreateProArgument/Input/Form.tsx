...
export const SERVICE_PRO_PRO_VIEW_EDIT_CREATE_PRO_ARGUMENT_INPUT_FORM_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_PRO_PRO_VIEW_EDIT_CREATE_PRO_ARGUMENT_INPUT_FORM_ACTIONS_HOOK';
...
export default function ServiceProPro_View_EditCreateProArgumentInputForm(
  ...
  const createProArgumentForProAction = async () => {
    try {
      setIsLoading(true);
      await serviceProServiceImpl.createProArgument(ownerData, cleanUpPayload(payloadDiff.current));
      if (customActions?.postCreateProArgumentForProAction) {
        await customActions.postCreateProArgumentForProAction(onSubmit, onClose);
      } else {
        showSuccessSnack(
  ...
  const getTemplateAction = async (): Promise<JudoRestResponse<CreateArgumentInput>> => {
    try {
      setIsLoading(true);
      const response = await serviceProServiceImpl.getTemplateOnCreateProArgument();
