...
import { useServiceConConsRelationViewPage } from '~/dialogs/Service/Con/Cons/Relation/View/Page';
...
import { serviceProServiceForConsImpl } from '~/services/data-axios';
...
export const convertServiceProConsRelationViewPagePayload = (attributeName: keyof ServiceCon, value: any): any => {
...
// Name: service::Pro::cons::Relation::View::Page
export default function ServiceProConsRelationViewPage(props: ServiceProConsRelationViewPageProps) {
  ...
  const storeDiff: (attributeName: keyof ServiceCon, value: any) => void = useCallback(
    (attributeName: keyof ServiceCon, value: any) => {
      payloadDiff.current![attributeName] = convertServiceProConsRelationViewPagePayload(attributeName, value);
      setData((prevData) => ({
        ...prevData,
        [attributeName]: value,
      }));
  ...
  const serviceConCon_View_EditRefresh = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceProServiceForConsImpl.refresh(ownerData, pageQueryCustomizer);
