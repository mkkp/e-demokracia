...
export const SERVICE_YES_NO_VOTE_DEFINITION_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_YES_NO_VOTE_DEFINITION_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceYesNoVoteDefinitionOwnerRelationViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceServiceUser>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceYesNoVoteDefinitionOwnerRelationViewPage
            ownerData={ownerData}
...
export const convertServiceYesNoVoteDefinitionOwnerRelationViewPagePayload = (
...
export interface ServiceYesNoVoteDefinitionOwnerRelationViewPageProps {
  ownerData: any;
...
// XMIID: User/(esm/_DxjSkHz5Ee6Q9LyUVjs1Qw)/RelationFeatureView
// Name: service::YesNoVoteDefinition::owner::RelationViewPage
export default function ServiceYesNoVoteDefinitionOwnerRelationViewPage(
  props: ServiceYesNoVoteDefinitionOwnerRelationViewPageProps,
) {
  ...
  const storeDiff: (attributeName: keyof ServiceServiceUser, value: any) => void = useCallback(
    (attributeName: keyof ServiceServiceUser, value: any) => {
      payloadDiff.current![attributeName] = convertServiceYesNoVoteDefinitionOwnerRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceServiceUserServiceUser_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_YES_NO_VOTE_DEFINITION_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const residentCityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoVoteDefinitionServiceForOwnerImpl.getRangeForResidentCity(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const residentCountyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoVoteDefinitionServiceForOwnerImpl.getRangeForResidentCounty(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const residentDistrictAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoVoteDefinitionServiceForOwnerImpl.getRangeForResidentDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceYesNoVoteDefinitionServiceForOwnerImpl.refresh(ownerData, getPageQueryCustomizer());
  ...
  return (
    <ServiceServiceUserServiceUser_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_DxjSkHz5Ee6Q9LyUVjs1Qw)/RelationFeatureView"
          data-page-name="service::YesNoVoteDefinition::owner::RelationViewPage"
        />
