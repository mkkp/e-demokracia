...
export const SERVICE_SERVICE_USER_RESIDENT_CITY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_SERVICE_USER_RESIDENT_CITY_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceServiceUserResidentCityRelationViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceCity>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceServiceUserResidentCityRelationViewPage
            ownerData={ownerData}
...
export const convertServiceServiceUserResidentCityRelationViewPagePayload = (
...
export interface ServiceServiceUserResidentCityRelationViewPageProps {
  ownerData: any;
...
// XMIID: User/(esm/_S-xWAIXoEe2kLcMqsIbMgQ)/RelationFeatureView
// Name: service::ServiceUser::residentCity::RelationViewPage
export default function ServiceServiceUserResidentCityRelationViewPage(
  props: ServiceServiceUserResidentCityRelationViewPageProps,
) {
  ...
  const storeDiff: (attributeName: keyof ServiceCity, value: any) => void = useCallback(
    (attributeName: keyof ServiceCity, value: any) => {
      payloadDiff.current![attributeName] = convertServiceServiceUserResidentCityRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceCityCity_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_SERVICE_USER_RESIDENT_CITY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const districtsDeleteAction = async (target: ServiceDistrictStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceServiceUserServiceForResidentCityImpl.deleteDistricts(target);
        if (!silentMode) {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceServiceUserServiceForResidentCityImpl.refresh(ownerData, getPageQueryCustomizer());
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceServiceUserServiceForResidentCityImpl.update(payloadDiff.current);
      if (res) {
  ...
  return (
    <ServiceCityCity_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_S-xWAIXoEe2kLcMqsIbMgQ)/RelationFeatureView"
          data-page-name="service::ServiceUser::residentCity::RelationViewPage"
        />
