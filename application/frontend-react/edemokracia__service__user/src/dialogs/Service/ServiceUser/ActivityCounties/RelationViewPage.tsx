...
export const SERVICE_SERVICE_USER_ACTIVITY_COUNTIES_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_SERVICE_USER_ACTIVITY_COUNTIES_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceServiceUserActivityCountiesRelationViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceCounty>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceServiceUserActivityCountiesRelationViewPage
            ownerData={ownerData}
...
export const convertServiceServiceUserActivityCountiesRelationViewPagePayload = (
...
export interface ServiceServiceUserActivityCountiesRelationViewPageProps {
  ownerData: any;
...
// Name: service::ServiceUser::activityCounties::RelationViewPage
export default function ServiceServiceUserActivityCountiesRelationViewPage(
  props: ServiceServiceUserActivityCountiesRelationViewPageProps,
) {
  ...
  const storeDiff: (attributeName: keyof ServiceCounty, value: any) => void = useCallback(
    (attributeName: keyof ServiceCounty, value: any) => {
      payloadDiff.current![attributeName] = convertServiceServiceUserActivityCountiesRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceCountyCounty_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_SERVICE_USER_ACTIVITY_COUNTIES_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const citiesDeleteAction = async (target: ServiceCityStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceServiceUserServiceForActivityCountiesImpl.deleteCities(target);
        if (!silentMode) {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceServiceUserServiceForActivityCountiesImpl.refresh(
        ownerData,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceServiceUserServiceForActivityCountiesImpl.update(payloadDiff.current);
      if (res) {
  ...
  return (
    <ServiceCountyCounty_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_uAEOkIXnEe2kLcMqsIbMgQ)/RelationFeatureView"
          data-page-name="service::ServiceUser::activityCounties::RelationViewPage"
        />
