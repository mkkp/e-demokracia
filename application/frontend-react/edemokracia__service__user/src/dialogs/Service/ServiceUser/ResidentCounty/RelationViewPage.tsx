...
export const SERVICE_SERVICE_USER_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_SERVICE_USER_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceServiceUserResidentCountyRelationViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceCounty>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceServiceUserResidentCountyRelationViewPage
            ownerData={ownerData}
...
export const convertServiceServiceUserResidentCountyRelationViewPagePayload = (
...
export interface ServiceServiceUserResidentCountyRelationViewPageProps {
  ownerData: any;
...
// XMIID: User/(esm/_z6WbsIXnEe2kLcMqsIbMgQ)/RelationFeatureView
// Name: service::ServiceUser::residentCounty::RelationViewPage
export default function ServiceServiceUserResidentCountyRelationViewPage(
  props: ServiceServiceUserResidentCountyRelationViewPageProps,
) {
  ...
  const storeDiff: (attributeName: keyof ServiceCounty, value: any) => void = useCallback(
    (attributeName: keyof ServiceCounty, value: any) => {
      payloadDiff.current![attributeName] = convertServiceServiceUserResidentCountyRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceCountyCounty_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_SERVICE_USER_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const citiesDeleteAction = async (target: ServiceCityStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceServiceUserServiceForResidentCountyImpl.deleteCities(target);
        if (!silentMode) {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceServiceUserServiceForResidentCountyImpl.refresh(
        ownerData,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceServiceUserServiceForResidentCountyImpl.update(payloadDiff.current);
      if (res) {
  ...
  return (
    <ServiceCountyCounty_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_z6WbsIXnEe2kLcMqsIbMgQ)/RelationFeatureView"
          data-page-name="service::ServiceUser::residentCounty::RelationViewPage"
        />
