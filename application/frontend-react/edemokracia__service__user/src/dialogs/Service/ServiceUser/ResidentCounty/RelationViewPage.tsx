...
export default function ServiceServiceUserResidentCountyRelationViewPage(
  ...
  const refreshAction = async (queryCustomizer: ServiceCountyQueryCustomizer): Promise<ServiceCountyStored> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceServiceUserServiceForResidentCountyImpl.refresh(ownerData, getPageQueryCustomizer());
      setData(result);
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const res = await serviceServiceUserServiceForResidentCountyImpl.update(payloadDiff.current);
      if (res) {
