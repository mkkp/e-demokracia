...
export const SERVICE_SERVICE_USER_RESIDENT_DISTRICT_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_SERVICE_USER_RESIDENT_DISTRICT_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceServiceUserResidentDistrictRelationViewPagePayload = (
...
// Name: service::ServiceUser::residentDistrict::RelationViewPage
export default function ServiceServiceUserResidentDistrictRelationViewPage(
  ...
  const storeDiff: (attributeName: keyof ServiceDistrict, value: any) => void = useCallback(
    (attributeName: keyof ServiceDistrict, value: any) => {
      payloadDiff.current![attributeName] = convertServiceServiceUserResidentDistrictRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceServiceUserServiceForResidentDistrictImpl.refresh(
        ownerData,
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await serviceServiceUserServiceForResidentDistrictImpl.update(payloadDiff.current);
      if (res) {
    ...
    <ServiceDistrictDistrict_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          ...
          data-page-name="service::ServiceUser::residentDistrict::RelationViewPage"
        />
