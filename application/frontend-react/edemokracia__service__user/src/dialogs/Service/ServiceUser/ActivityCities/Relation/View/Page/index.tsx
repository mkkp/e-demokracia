...
// Name: service::ServiceUser::activityCities::Relation::View::Page
export default function ServiceServiceUserActivityCitiesRelationViewPage(
  ...
  const serviceCityCity_View_EditRefresh = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceServiceUserServiceForActivityCitiesImpl.refresh(ownerData, pageQueryCustomizer);
  ...
  const serviceCityCity_View_EditUpdate = async () => {
    ...
    try {
      const res = await serviceServiceUserServiceForActivityCitiesImpl.update(payloadDiff.current);
