...
export default function ServiceSelectAnswerVoteEntryOwnerRelationViewPage(
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceSelectAnswerVoteEntryServiceForOwnerImpl.refresh(ownerData, getPageQueryCustomizer());
      setData(result);
  ...
  const residentCityAutocompleteRangeAction = async (
    ...
    try {
      return serviceSelectAnswerVoteEntryServiceForOwnerImpl.getRangeForResidentCity(
        cleanUpPayload(data),
        queryCustomizer,
      );
    } catch (error) {
  ...
  const residentCountyAutocompleteRangeAction = async (
    ...
    try {
      return serviceSelectAnswerVoteEntryServiceForOwnerImpl.getRangeForResidentCounty(
        cleanUpPayload(data),
        queryCustomizer,
      );
    } catch (error) {
  ...
  const residentDistrictAutocompleteRangeAction = async (
    ...
    try {
      return serviceSelectAnswerVoteEntryServiceForOwnerImpl.getRangeForResidentDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
    } catch (error) {
