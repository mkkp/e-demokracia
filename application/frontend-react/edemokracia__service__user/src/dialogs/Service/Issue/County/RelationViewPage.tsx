...
export const SERVICE_ISSUE_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_ISSUE_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const convertServiceIssueCountyRelationViewPagePayload = (
...
// Name: service::Issue::county::RelationViewPage
export default function ServiceIssueCountyRelationViewPage(props: ServiceIssueCountyRelationViewPageProps) {
  ...
  const storeDiff: (attributeName: keyof ServiceCounty, value: any) => void = useCallback(
    (attributeName: keyof ServiceCounty, value: any) => {
      payloadDiff.current![attributeName] = convertServiceIssueCountyRelationViewPagePayload(attributeName, value);
      setData((prevData) => ({
        ...prevData,
        [attributeName]: value,
      }));
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceIssueServiceForCountyImpl.refresh(ownerData, getPageQueryCustomizer());
    ...
    <ServiceCountyCounty_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          ...
          data-page-name="service::Issue::county::RelationViewPage"
        />
