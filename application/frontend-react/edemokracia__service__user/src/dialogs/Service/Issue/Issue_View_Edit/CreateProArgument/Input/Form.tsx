...
export const SERVICE_ISSUE_ISSUE_VIEW_EDIT_CREATE_PRO_ARGUMENT_INPUT_FORM_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_ISSUE_ISSUE_VIEW_EDIT_CREATE_PRO_ARGUMENT_INPUT_FORM_ACTIONS_HOOK';
...
export default function ServiceIssueIssue_View_EditCreateProArgumentInputForm(
  ...
  const createProArgumentForIssueAction = async () => {
    try {
      setIsLoading(true);
      await serviceIssueServiceImpl.createProArgument(ownerData, cleanUpPayload(payloadDiff.current));
  ...
  const getTemplateAction = async (): Promise<JudoRestResponse<CreateArgumentInput>> => {
    try {
      setIsLoading(true);
      const response = await serviceIssueServiceImpl.getTemplateOnCreateProArgument();
