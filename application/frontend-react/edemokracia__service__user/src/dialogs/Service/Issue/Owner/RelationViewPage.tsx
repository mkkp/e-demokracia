...
export default function ServiceIssueOwnerRelationViewPage(props: ServiceIssueOwnerRelationViewPageProps) {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceIssueServiceForOwnerImpl.refresh(ownerData, getPageQueryCustomizer());
  ...
  const residentCityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceIssueServiceForOwnerImpl.getRangeForResidentCity(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const residentCountyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceIssueServiceForOwnerImpl.getRangeForResidentCounty(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const residentDistrictAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceIssueServiceForOwnerImpl.getRangeForResidentDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
