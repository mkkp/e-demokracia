...
export const SERVICE_ISSUE_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_ISSUE_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export default function ServiceIssueOwnerRelationViewPage(props: ServiceIssueOwnerRelationViewPageProps) {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceIssueServiceForOwnerImpl.refresh(ownerData, getPageQueryCustomizer());
      setData(result);
  ...
  const residentCityAutocompleteRangeAction = async (
    ...
    try {
      return serviceIssueServiceForOwnerImpl.getRangeForResidentCity(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const residentCountyAutocompleteRangeAction = async (
    ...
    try {
      return serviceIssueServiceForOwnerImpl.getRangeForResidentCounty(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const residentDistrictAutocompleteRangeAction = async (
    ...
    try {
      return serviceIssueServiceForOwnerImpl.getRangeForResidentDistrict(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
