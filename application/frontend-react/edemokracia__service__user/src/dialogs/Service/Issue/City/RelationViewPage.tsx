...
export const SERVICE_ISSUE_CITY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_ISSUE_CITY_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceIssueCityRelationViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceCity>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceIssueCityRelationViewPage
            ownerData={ownerData}
...
export const convertServiceIssueCityRelationViewPagePayload = (attributeName: keyof ServiceCity, value: any): any => {
  return value;
};
...
export interface ServiceIssueCityRelationViewPageProps {
  ownerData: any;
...
// XMIID: User/(esm/__tg8wNvTEe2Bgcx6em3jZg)/RelationFeatureView
// Name: service::Issue::city::RelationViewPage
export default function ServiceIssueCityRelationViewPage(props: ServiceIssueCityRelationViewPageProps) {
  ...
  const storeDiff: (attributeName: keyof ServiceCity, value: any) => void = useCallback(
    (attributeName: keyof ServiceCity, value: any) => {
      payloadDiff.current![attributeName] = convertServiceIssueCityRelationViewPagePayload(attributeName, value);
      setData((prevData) => ({
        ...prevData,
        [attributeName]: value,
      }));
  ...
  const { service: customActionsHook } = useTrackService<ServiceCityCity_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_ISSUE_CITY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const districtsDeleteAction = async (target: ServiceDistrictStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceIssueServiceForCityImpl.deleteDistricts(target);
        if (!silentMode) {
  ...
  return (
    <ServiceCityCity_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/__tg8wNvTEe2Bgcx6em3jZg)/RelationFeatureView"
          data-page-name="service::Issue::city::RelationViewPage"
        />
