...
import { ServiceIssueServiceForCreatedByImpl } from '~/services/data-axios/ServiceIssueServiceForCreatedByImpl';
...
export const SERVICE_ISSUE_CREATED_BY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_ISSUE_CREATED_BY_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceIssueCreatedByRelationViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceServiceUser>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceIssueCreatedByRelationViewPage
            ownerData={ownerData}
...
export const convertServiceIssueCreatedByRelationViewPagePayload = (
...
export interface ServiceIssueCreatedByRelationViewPageProps {
  ownerData: any;
...
// XMIID: User/(esm/_rZFZ4I1pEe29qs15q2b6yw)/RelationFeatureView
// Name: service::Issue::createdBy::RelationViewPage
export default function ServiceIssueCreatedByRelationViewPage(props: ServiceIssueCreatedByRelationViewPageProps) {
  const { ownerData, templateDataOverride, onClose, onSubmit, isDraft, ownerValidation } = props;

  // Services
  const serviceIssueServiceForCreatedByImpl = useMemo(
  ...
  const storeDiff: (attributeName: keyof ServiceServiceUser, value: any) => void = useCallback(
    (attributeName: keyof ServiceServiceUser, value: any) => {
      payloadDiff.current![attributeName] = convertServiceIssueCreatedByRelationViewPagePayload(attributeName, value);
      setData((prevData) => ({
        ...prevData,
        [attributeName]: value,
      }));
  ...
  const { service: customActionsHook } = useTrackService<ServiceServiceUserServiceUser_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_ISSUE_CREATED_BY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceIssueServiceForCreatedByImpl.refresh(ownerData, getPageQueryCustomizer());
      setData(result);
  ...
  const residentCityAutocompleteRangeAction = async (
    ...
    try {
      return serviceIssueServiceForCreatedByImpl.getRangeForResidentCity(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const residentCountyAutocompleteRangeAction = async (
    ...
    try {
      return serviceIssueServiceForCreatedByImpl.getRangeForResidentCounty(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const residentDistrictAutocompleteRangeAction = async (
    ...
    try {
      return serviceIssueServiceForCreatedByImpl.getRangeForResidentDistrict(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  return (
    <ServiceServiceUserServiceUser_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_rZFZ4I1pEe29qs15q2b6yw)/RelationFeatureView"
          data-page-name="service::Issue::createdBy::RelationViewPage"
        />
