...
// Name: service::County::cities::Relation::View::Page
export default function ServiceCountyCitiesRelationViewPage(props: ServiceCountyCitiesRelationViewPageProps) {
  ...
  const serviceCityCity_View_EditRefresh = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceCountyServiceForCitiesImpl.refresh(ownerData, pageQueryCustomizer);
  ...
  const serviceCityCity_View_EditDelete = async () => {
    try {
  ...
  const actions: ServiceCityCity_View_EditDialogActions = {
    ...
    serviceCityCity_View_EditDelete,
