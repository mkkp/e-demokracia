...
import { UserServiceForRatingVoteEntriesImpl } from '~/services/data-axios/UserServiceForRatingVoteEntriesImpl';
...
export const SERVICE_USER_RATING_VOTE_ENTRIES_ACCESS_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_RATING_VOTE_ENTRIES_ACCESS_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceUserRatingVoteEntriesAccessViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceRatingVoteEntry>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceUserRatingVoteEntriesAccessViewPage
            ownerData={ownerData}
...
export const convertServiceUserRatingVoteEntriesAccessViewPagePayload = (
...
export interface ServiceUserRatingVoteEntriesAccessViewPageProps {
  ownerData: any;
...
// XMIID: User/(esm/_8Ip-IM6zEe6ddqBfFzCBZg)/AccessViewPageDefinition
// Name: service::User::ratingVoteEntries::AccessViewPage
export default function ServiceUserRatingVoteEntriesAccessViewPage(
  props: ServiceUserRatingVoteEntriesAccessViewPageProps,
) {
  const { ownerData, templateDataOverride, onClose, onSubmit, isDraft, ownerValidation } = props;

  // Services
  const userServiceForRatingVoteEntriesImpl = useMemo(
  ...
  const storeDiff: (attributeName: keyof ServiceRatingVoteEntry, value: any) => void = useCallback(
    (attributeName: keyof ServiceRatingVoteEntry, value: any) => {
      payloadDiff.current![attributeName] = convertServiceUserRatingVoteEntriesAccessViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceRatingVoteEntryRatingVoteEntry_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_RATING_VOTE_ENTRIES_ACCESS_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await userServiceForRatingVoteEntriesImpl.getRangeForOwner(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await userServiceForRatingVoteEntriesImpl.refresh(ownerData, getPageQueryCustomizer());
  ...
  return (
    <ServiceRatingVoteEntryRatingVoteEntry_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_8Ip-IM6zEe6ddqBfFzCBZg)/AccessViewPageDefinition"
          data-page-name="service::User::ratingVoteEntries::AccessViewPage"
        />
