...
import { UserServiceForYesNoVoteEntriesImpl } from '~/services/data-axios/UserServiceForYesNoVoteEntriesImpl';
...
export const SERVICE_USER_YES_NO_VOTE_ENTRIES_ACCESS_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_YES_NO_VOTE_ENTRIES_ACCESS_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceUserYesNoVoteEntriesAccessViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceYesNoVoteEntry>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceUserYesNoVoteEntriesAccessViewPage
            ownerData={ownerData}
...
export const convertServiceUserYesNoVoteEntriesAccessViewPagePayload = (
...
export interface ServiceUserYesNoVoteEntriesAccessViewPageProps {
  ownerData: any;
  ...
  onSubmit: (result?: ServiceYesNoVoteEntryStored, isDraft?: boolean) => Promise<void>;
}

// XMIID: User/(esm/_4bqvAM6zEe6ddqBfFzCBZg)/AccessViewPageDefinition
// Name: service::User::yesNoVoteEntries::AccessViewPage
export default function ServiceUserYesNoVoteEntriesAccessViewPage(
  props: ServiceUserYesNoVoteEntriesAccessViewPageProps,
) {
  const { ownerData, templateDataOverride, onClose, onSubmit, isDraft, ownerValidation } = props;

  // Services
  const userServiceForYesNoVoteEntriesImpl = useMemo(
  ...
  const storeDiff: (attributeName: keyof ServiceYesNoVoteEntry, value: any) => void = useCallback(
    (attributeName: keyof ServiceYesNoVoteEntry, value: any) => {
      payloadDiff.current![attributeName] = convertServiceUserYesNoVoteEntriesAccessViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceYesNoVoteEntryYesNoVoteEntry_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_YES_NO_VOTE_ENTRIES_ACCESS_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await userServiceForYesNoVoteEntriesImpl.refresh(ownerData, getPageQueryCustomizer());
  ...
  const updateAction = async () => {
    setIsLoading(true);
    try {
      const { data: res } = await userServiceForYesNoVoteEntriesImpl.update(payloadDiff.current);
      if (res) {
        showSuccessSnack(t('judo.action.save.success', { defaultValue: 'Changes saved' }));
        setValidation(new Map<keyof ServiceYesNoVoteEntry, string>());
        setEditMode(false);
        await actions.refreshAction!(getPageQueryCustomizer());
      }
    } catch (error) {
      handleError<ServiceYesNoVoteEntry>(error, { setValidation }, data);
    } finally {
      setIsLoading(false);
    }
  };
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await userServiceForYesNoVoteEntriesImpl.getRangeForOwner(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  return (
    <ServiceYesNoVoteEntryYesNoVoteEntry_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_4bqvAM6zEe6ddqBfFzCBZg)/AccessViewPageDefinition"
          data-page-name="service::User::yesNoVoteEntries::AccessViewPage"
        />
