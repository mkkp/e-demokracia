...
export const SERVICE_CREATE_ISSUE_INPUT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_CREATE_ISSUE_INPUT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceCreateIssueInputCountyRelationViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceCounty>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceCreateIssueInputCountyRelationViewPage
            ownerData={ownerData}
...
export const convertServiceCreateIssueInputCountyRelationViewPagePayload = (
...
export interface ServiceCreateIssueInputCountyRelationViewPageProps {
  ownerData: any;
...
// XMIID: User/(esm/_7lJ44NvWEe2Bgcx6em3jZg)/RelationFeatureView
// Name: service::CreateIssueInput::county::RelationViewPage
export default function ServiceCreateIssueInputCountyRelationViewPage(
  props: ServiceCreateIssueInputCountyRelationViewPageProps,
) {
  ...
  const storeDiff: (attributeName: keyof ServiceCounty, value: any) => void = useCallback(
    (attributeName: keyof ServiceCounty, value: any) => {
      payloadDiff.current![attributeName] = convertServiceCreateIssueInputCountyRelationViewPagePayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<ServiceCountyCounty_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_CREATE_ISSUE_INPUT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const citiesDeleteAction = async (target: ServiceCityStored, silentMode?: boolean) => {
    try {
      const confirmed = !silentMode
        ...
        : true;
      if (confirmed) {
        await serviceCreateIssueInputServiceForCountyImpl.deleteCities(target);
        if (!silentMode) {
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceCreateIssueInputServiceForCountyImpl.refresh(ownerData, getPageQueryCustomizer());
  ...
  return (
    <ServiceCountyCounty_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_7lJ44NvWEe2Bgcx6em3jZg)/RelationFeatureView"
          data-page-name="service::CreateIssueInput::county::RelationViewPage"
        />
