...
export const SERVICE_USER_PROFILE_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_PROFILE_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
// XMIID: User/(esm/_mSi7YFvREe6jm_SkPSYEYw)/RelationFeatureView
// Name: service::UserProfile::residentCounty::RelationViewPage
export default function ServiceUserProfileResidentCountyRelationViewPage(
  ...
  const { service: customActionsHook } = useTrackService<ServiceCountyCounty_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_PROFILE_RESIDENT_COUNTY_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const refreshAction = async (queryCustomizer: ServiceCountyQueryCustomizer): Promise<ServiceCountyStored> => {
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceUserProfileServiceForResidentCountyImpl.refresh(ownerData, getPageQueryCustomizer());
      setData(result);
  ...
  return (
    <ServiceCountyCounty_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_mSi7YFvREe6jm_SkPSYEYw)/RelationFeatureView"
          data-page-name="service::UserProfile::residentCounty::RelationViewPage"
        />
