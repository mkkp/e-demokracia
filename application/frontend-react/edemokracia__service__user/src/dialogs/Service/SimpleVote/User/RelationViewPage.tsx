...
import { ServiceSimpleVoteServiceForUserImpl } from '~/services/data-axios/ServiceSimpleVoteServiceForUserImpl';
...
export const SERVICE_SIMPLE_VOTE_USER_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_SIMPLE_VOTE_USER_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
export const useServiceSimpleVoteUserRelationViewPage = (): ((
  ownerData: any,
  templateDataOverride?: Partial<ServiceServiceUser>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceSimpleVoteUserRelationViewPage
            ownerData={ownerData}
...
export const convertServiceSimpleVoteUserRelationViewPagePayload = (
...
export interface ServiceSimpleVoteUserRelationViewPageProps {
  ownerData: any;
...
// XMIID: User/(esm/_qcGgsGksEe25ONJ3V89cVA)/RelationFeatureView
// Name: service::SimpleVote::user::RelationViewPage
export default function ServiceSimpleVoteUserRelationViewPage(props: ServiceSimpleVoteUserRelationViewPageProps) {
  const { ownerData, templateDataOverride, onClose, onSubmit, isDraft, ownerValidation } = props;

  // Services
  const serviceSimpleVoteServiceForUserImpl = useMemo(
  ...
  const storeDiff: (attributeName: keyof ServiceServiceUser, value: any) => void = useCallback(
    (attributeName: keyof ServiceServiceUser, value: any) => {
      payloadDiff.current![attributeName] = convertServiceSimpleVoteUserRelationViewPagePayload(attributeName, value);
      setData((prevData) => ({
        ...prevData,
        [attributeName]: value,
      }));
  ...
  const { service: customActionsHook } = useTrackService<ServiceServiceUserServiceUser_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_SIMPLE_VOTE_USER_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const residentCityAutocompleteRangeAction = async (
    ...
    try {
      return serviceSimpleVoteServiceForUserImpl.getRangeForResidentCity(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const residentCountyAutocompleteRangeAction = async (
    ...
    try {
      return serviceSimpleVoteServiceForUserImpl.getRangeForResidentCounty(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const residentDistrictAutocompleteRangeAction = async (
    ...
    try {
      return serviceSimpleVoteServiceForUserImpl.getRangeForResidentDistrict(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceSimpleVoteServiceForUserImpl.refresh(ownerData, getPageQueryCustomizer());
      setData(result);
  ...
  return (
    <ServiceServiceUserServiceUser_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_qcGgsGksEe25ONJ3V89cVA)/RelationFeatureView"
          data-page-name="service::SimpleVote::user::RelationViewPage"
        />
