...
// Name: service::YesNoAbstainVoteDefinition::voteEntries::RelationViewPage
export default function ServiceYesNoAbstainVoteDefinitionVoteEntriesRelationViewPage(
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceYesNoAbstainVoteDefinitionServiceForVoteEntriesImpl.refresh(
        ownerData,
  ...
  const ownerAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceYesNoAbstainVoteDefinitionServiceForVoteEntriesImpl.getRangeForOwner(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
