...
export const SERVICE_CON_CON_VIEW_EDIT_CREATE_CON_ARGUMENT_INPUT_FORM_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_CON_CON_VIEW_EDIT_CREATE_CON_ARGUMENT_INPUT_FORM_ACTIONS_HOOK';
...
export default function ServiceConCon_View_EditCreateConArgumentInputForm(
  ...
  const createConArgumentForConAction = async () => {
    try {
      setIsLoading(true);
      await serviceConServiceImpl.createConArgument(ownerData, cleanUpPayload(payloadDiff.current));
      if (customActions?.postCreateConArgumentForConAction) {
        await customActions.postCreateConArgumentForConAction(onSubmit, onClose);
      } else {
        showSuccessSnack(
  ...
  const getTemplateAction = async (): Promise<JudoRestResponse<CreateArgumentInput>> => {
    try {
      setIsLoading(true);
      const response = await serviceConServiceImpl.getTemplateOnCreateConArgument();
