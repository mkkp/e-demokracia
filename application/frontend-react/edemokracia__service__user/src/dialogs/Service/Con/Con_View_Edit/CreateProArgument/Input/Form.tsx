...
export const SERVICE_CON_CON_VIEW_EDIT_CREATE_PRO_ARGUMENT_INPUT_FORM_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_CON_CON_VIEW_EDIT_CREATE_PRO_ARGUMENT_INPUT_FORM_ACTIONS_HOOK';
...
export const useServiceConCon_View_EditCreateProArgumentInputForm = (): ((
  ownerData: any,
  templateDataOverride?: Partial<CreateArgumentInput>,
  isDraft?: boolean,
  ...
  return (
    ...
    new Promise((resolve) => {
      createDialog({
        ...
        children: (
          <ServiceConCon_View_EditCreateProArgumentInputForm
            ownerData={ownerData}
...
export const convertServiceConCon_View_EditCreateProArgumentInputFormPayload = (
...
export interface ServiceConCon_View_EditCreateProArgumentInputFormProps {
  ownerData: any;
...
// XMIID: User/(esm/_DBZYMHjsEe6cB8og8p0UuQ)/OperationUnmappedInputPageDefinition
// Name: service::Con::Con_View_Edit::createProArgument::Input::Form
export default function ServiceConCon_View_EditCreateProArgumentInputForm(
  props: ServiceConCon_View_EditCreateProArgumentInputFormProps,
) {
  ...
  const storeDiff: (attributeName: keyof CreateArgumentInput, value: any) => void = useCallback(
    (attributeName: keyof CreateArgumentInput, value: any) => {
      payloadDiff.current![attributeName] = convertServiceConCon_View_EditCreateProArgumentInputFormPayload(
        attributeName,
        value,
      );
      setData((prevData) => ({
  ...
  const { service: customActionsHook } = useTrackService<CreateArgumentInputCreateArgumentInput_FormActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_CON_CON_VIEW_EDIT_CREATE_PRO_ARGUMENT_INPUT_FORM_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const createProArgumentForConAction = async () => {
    try {
      setIsLoading(true);
      await serviceConServiceImpl.createProArgument(ownerData, cleanUpPayload(payloadDiff.current));
      if (customActions?.postCreateProArgumentForConAction) {
        await customActions.postCreateProArgumentForConAction(onSubmit, onClose);
      } else {
        showSuccessSnack(
  ...
  const getTemplateAction = async (): Promise<JudoRestResponse<CreateArgumentInput>> => {
    try {
      setIsLoading(true);
      const response = await serviceConServiceImpl.getTemplateOnCreateProArgument();
  ...
  return (
    <CreateArgumentInputCreateArgumentInput_FormViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_DBZYMHjsEe6cB8og8p0UuQ)/OperationUnmappedInputPageDefinition"
          data-page-name="service::Con::Con_View_Edit::createProArgument::Input::Form"
        />
