...
export default function ServiceConCon_View_EditCreateProArgumentInputForm(
  ...
  const createProArgumentForConAction = async () => {
    try {
      setIsLoading(true);
      await serviceConServiceImpl.createProArgument(ownerData, data);

      if (customActions?.postCreateProArgumentForConAction) {
        await customActions.postCreateProArgumentForConAction(
          onSubmit,
          onClose,
        );
      } else {
  ...
  const getTemplateAction = async (): Promise<CreateArgumentInput> => {
    try {
      setIsLoading(true);
      const result = await serviceConServiceImpl.getTemplateForCreateProArgument();
