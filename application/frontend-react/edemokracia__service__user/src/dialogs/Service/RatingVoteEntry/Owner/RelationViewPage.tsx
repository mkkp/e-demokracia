...
export const SERVICE_RATING_VOTE_ENTRY_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_RATING_VOTE_ENTRY_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK';
...
// XMIID: User/(esm/_ap8zwFslEe6Mx9dH3yj5gQ)/RelationFeatureView
// Name: service::RatingVoteEntry::owner::RelationViewPage
export default function ServiceRatingVoteEntryOwnerRelationViewPage(
  ...
  const { service: customActionsHook } = useTrackService<ServiceServiceUserServiceUser_View_EditActionsHook>(
    `(${OBJECTCLASS}=${SERVICE_RATING_VOTE_ENTRY_OWNER_RELATION_VIEW_PAGE_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const result = await serviceRatingVoteEntryServiceForOwnerImpl.refresh(ownerData, getPageQueryCustomizer());
      setData(result);
  ...
  const residentCityAutocompleteRangeAction = async (
    ...
    try {
      return serviceRatingVoteEntryServiceForOwnerImpl.getRangeForResidentCity(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
  ...
  const residentCountyAutocompleteRangeAction = async (
    ...
    try {
      return serviceRatingVoteEntryServiceForOwnerImpl.getRangeForResidentCounty(cleanUpPayload(data), queryCustomizer);
    } catch (error) {
      handleError(error);
      return Promise.resolve([]);
    }
  ...
  const residentDistrictAutocompleteRangeAction = async (
    ...
    try {
      return serviceRatingVoteEntryServiceForOwnerImpl.getRangeForResidentDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
    } catch (error) {
  ...
  return (
    <ServiceServiceUserServiceUser_View_EditViewModelContext.Provider value={viewModel}>
      <Suspense>
        <div
          id="User/(esm/_ap8zwFslEe6Mx9dH3yj5gQ)/RelationFeatureView"
          data-page-name="service::RatingVoteEntry::owner::RelationViewPage"
        />
