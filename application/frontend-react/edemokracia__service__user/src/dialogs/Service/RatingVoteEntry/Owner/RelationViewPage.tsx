...
export default function ServiceRatingVoteEntryOwnerRelationViewPage(
  ...
  const refreshAction = async (
    ...
    try {
      setIsLoading(true);
      setEditMode(false);
      const response = await serviceRatingVoteEntryServiceForOwnerImpl.refresh(ownerData, getPageQueryCustomizer());
  ...
  const residentCityAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceRatingVoteEntryServiceForOwnerImpl.getRangeForResidentCity(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const residentCountyAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceRatingVoteEntryServiceForOwnerImpl.getRangeForResidentCounty(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
  ...
  const residentDistrictAutocompleteRangeAction = async (
    ...
    try {
      const { data: result } = await serviceRatingVoteEntryServiceForOwnerImpl.getRangeForResidentDistrict(
        cleanUpPayload(data),
        queryCustomizer,
      );
      return result;
