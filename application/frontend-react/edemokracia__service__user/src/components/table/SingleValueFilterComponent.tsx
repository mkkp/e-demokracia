...
export const SingleValueFilterComponent: FC<SingleValueFilterComponentProps<any>> = ({ filters, onFiltersChange, filterType, operator, attributeName, label, col }) => {
  const fallbackFilter: Filter = buildFilter(filterType, operator, attributeName as string, null);
 
  const [filter, setFilter] = useState<Filter>(filters?.find(f => 
    f.filterOption.attributeName === attributeName &&
    f.filterOption.filterType === filterType &&
    f.filterBy.operator == operator) || fallbackFilter);
