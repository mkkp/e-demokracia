...
export const getOperation = (filterOption: FilterOption, operator: string) => {
...
export enum _FilterOperationType {
  EQUAL = 'equal',
  NOT_EQUAL = 'notEqual',
  IS_EMPTY = 'isEmpty',
...
const getEnumKeyByValue = (enumerated: any, value: string) => {
...
export const getOperationEnumValue = (filter: Filter, operator: string) => {
  return getOperation(filter.filterOption, operator);
};

export const getOperationFromFilterOperationType = (filterOption: FilterOption, opType?: string): Operation => {
  if (opType == undefined) {
    return getOperationFromFilterOperationType(filterOption, _FilterOperationType.EQUAL);
  }
  const type = (opType || '') as _FilterOperationType;
  let op = opType;
  if (
    filterOption.filterType === FilterType.boolean ||
    filterOption.filterType === FilterType.trinaryLogic ||
    filterOption.filterType === FilterType.enumeration
  ) {
    if (type === _FilterOperationType.EQUAL) {
      op = 'equals';
    } else if (type === _FilterOperationType.NOT_EQUAL) {
      op = 'notEquals';
    }
  }
  const operation =  getOperation(filterOption, op);
  return operation;
};
...
export function mapPersistentTableFiltersToFilters<T>(
  ...
  for (const opt of filterOptions) {
    if (opt.alwaysShown) {
      const key = opt.attributeName as string;
      if (values.hasOwnProperty(opt.attributeName))
        filters.push(
          buildFilter(
            opt.filterType,
            getOperationFromFilterOperationType(opt, opt.defaultOperationType),
            opt.attributeName,
            values[key],
          ),
        );
