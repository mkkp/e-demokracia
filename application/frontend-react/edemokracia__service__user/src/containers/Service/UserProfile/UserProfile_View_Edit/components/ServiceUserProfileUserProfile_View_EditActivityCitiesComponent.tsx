...
export const SERVICE_USER_PROFILE_USER_PROFILE_VIEW_EDIT_ACTIVITY_CITIES_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY =
  'ServiceUserProfileUserProfile_View_EditActivityCitiesComponentSidekickComponent';
...
export function ServiceUserProfileUserProfile_View_EditActivityCitiesComponent(
  ...
  const sidekickComponentFilter = `(&(${OBJECTCLASS}=${CUSTOM_VISUAL_ELEMENT_INTERFACE_KEY})(component=${SERVICE_USER_PROFILE_USER_PROFILE_VIEW_EDIT_ACTIVITY_CITIES_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY}))`;
  const filterModelKey = `User/(esm/_fsW_rVvTEe6jm_SkPSYEYw)/TabularReferenceFieldRelationDefinedTable-${uniqueId}-${generatorUUIDv4}-filterModel`;
  const filtersKey = `User/(esm/_fsW_rVvTEe6jm_SkPSYEYw)/TabularReferenceFieldRelationDefinedTable-${uniqueId}-${generatorUUIDv4}-filters`;
  const rowsPerPageKey = `User/(esm/_fsW_rVvTEe6jm_SkPSYEYw)/TabularReferenceFieldRelationDefinedTable-${uniqueId}-${generatorUUIDv4}-rowsPerPage`;
  ...
  const representationColumn: GridColDef<ServiceCityStored> = {
    ...
    headerName: t('service.UserProfile.UserProfile_View_Edit.activityCities.representation', {
      defaultValue: 'City',
    }) as string,
  ...
  const additionalToolbarActions: Record<string, ElementType> = actions?.activityCitiesAdditionalToolbarButtons
    ? actions.activityCitiesAdditionalToolbarButtons(
        data,
        isLoading,
        getSelectedRows(),
        clearSelections,
  ...
  const filterOptions = useMemo<FilterOption[]>(
    () => [
      {
        id: 'User/(esm/_fsW_rlvTEe6jm_SkPSYEYw)/TableColumnFilter/(discriminator/User/(esm/_fsW_rVvTEe6jm_SkPSYEYw)/TabularReferenceFieldRelationDefinedTable)',
        ...
        onRowClick={
          actions.activityCitiesOpenPageAction
            ? async (params: GridRowParams<ServiceCityStored>) =>
                await actions.activityCitiesOpenPageAction!(params.row, false)
            : undefined
        }
        sortModel={sortModel}
              ...
              {actions.activityCitiesRefreshAction && true ? (
                <Button
                  ...
                  onClick={async () => {
                    ...
                    await actions.activityCitiesRefreshAction!(processedQueryCustomizer);
                  }}
                  disabled={isLoading}
                >
