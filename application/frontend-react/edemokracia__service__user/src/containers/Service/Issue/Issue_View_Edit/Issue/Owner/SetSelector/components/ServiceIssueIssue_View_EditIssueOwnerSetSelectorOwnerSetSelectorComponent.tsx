...
export interface ServiceIssueIssue_View_EditIssueOwnerSetSelectorOwnerSetSelectorComponentActionDefinitions {
...
export interface ServiceIssueIssue_View_EditIssueOwnerSetSelectorOwnerSetSelectorComponentProps {
  uniqueId: string;
  actions: ServiceIssueIssue_View_EditIssueOwnerSetSelectorOwnerSetSelectorComponentActionDefinitions;
...
export const SERVICE_ISSUE_ISSUE_VIEW_EDIT_ISSUE_OWNER_SET_SELECTOR_OWNER_SET_SELECTOR_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY =
  'ServiceIssueIssue_View_EditIssueOwnerSetSelectorOwnerSetSelectorComponentSidekickComponent';

// XMIID: User/(esm/_plsB8Id8Ee2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTable
// Name: owner::Set::Selector
export function ServiceIssueIssue_View_EditIssueOwnerSetSelectorOwnerSetSelectorComponent(
  props: ServiceIssueIssue_View_EditIssueOwnerSetSelectorOwnerSetSelectorComponentProps,
) {
  const {
    uniqueId,
    actions,
  ...
  const sidekickComponentFilter = `(&(${OBJECTCLASS}=${CUSTOM_VISUAL_ELEMENT_INTERFACE_KEY})(component=${SERVICE_ISSUE_ISSUE_VIEW_EDIT_ISSUE_OWNER_SET_SELECTOR_OWNER_SET_SELECTOR_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY}))`;
  const filterModelKey = `User/(esm/_plsB8Id8Ee2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTable-${uniqueId}-${generatorUUIDv4}-filterModel`;
  const filtersKey = `User/(esm/_plsB8Id8Ee2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTable-${uniqueId}-${generatorUUIDv4}-filters`;
  const rowsPerPageKey = `User/(esm/_plsB8Id8Ee2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTable-${uniqueId}-${generatorUUIDv4}-rowsPerPage`;
  ...
  const representationColumn: GridColDef<ServiceServiceUserStored> = {
    ...
    headerName: t('service.Issue.Issue_View_Edit.issue.owner.SetSelector.representation', {
  ...
  const effectiveTableColumns = useMemo(() => {
    const cols = [
      ...columns,
      ...columnsActionCalculator('User/(esm/_qYS-wGksEe25ONJ3V89cVA)/RelationType', rowActions, t, {
  ...
  const filterOptions = useMemo<FilterOption[]>(
    () => [
      {
        id: 'User/(esm/_pltQEId8Ee2kLcMqsIbMgQ)/TableColumnFilter/(discriminator/User/(esm/_plsB8Id8Ee2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTable)',
        attributeName: 'representation',
        label: t('service.Issue.Issue_View_Edit.issue.owner.SetSelector.representation', {
          defaultValue: 'Representation',
        }) as string,
          ...
          Toolbar: () => (
            <GridToolbarContainer>
              {actions.filterAction && true ? (
                <Button
                  id="User/(esm/_plsB8Id8Ee2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTableFilterButton"
                  startIcon={<MdiIcon path="filter" />}
                  variant={'text'}
                  onClick={async () => {
                    const filterResults = await actions.filterAction!(
                      'User/(esm/_plsB8Id8Ee2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTableFilterButton',
                      filterOptions,
                      filterModel,
                      filters,
                    );
              ...
              {actions.selectorRangeAction && true ? (
                <Button
                  id="User/(esm/_plsB8Id8Ee2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTableRefreshButton"
              ...
              {actions.openCreateFormAction && true ? (
                <Button
                  id="User/(esm/_plsB8Id8Ee2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTableCreateButton"
