...
export interface ServiceUserVoteDefinitionUserVoteDefinition_View_EditActiveVoteDefinitionsInResidentCityComponentActionDefinitions {
  activeVoteDefinitionsInResidentCityOpenAddSelectorAction?: () => Promise<void>;
  ...
  const rowActions: TableRowAction<ServiceVoteDefinitionStored>[] = [
    {
      ...
      label: t(
        'service.UserVoteDefinition.UserVoteDefinition_View_Edit.root.tabBar.ActiveVoteDefinitionsByOwnerResidentAreaGroup.tabBar.ActiveDebatesInResidentCityGroup.activeVoteDefinitionsInResidentCity.Remove',
        { defaultValue: 'Remove' },
      ) as string,
      icon: <MdiIcon path="link_off" />,
      disabled: (row: ServiceVoteDefinitionStored) => !isFormUpdateable() || isLoading,
      action: actions.activeVoteDefinitionsInResidentCityRemoveAction
        ? async (rowData) => {
            await actions.activeVoteDefinitionsInResidentCityRemoveAction!(rowData);
          }
        : undefined,
    },
    {
        ...
        components={{
          Toolbar: () => (
            <GridToolbarContainer>
              ...
              {actions.activeVoteDefinitionsInResidentCityOpenAddSelectorAction && isFormUpdateable() ? (
                <Button
                  id="User/(esm/_yjuNkF5MEe6vsex_cZNQbQ)/TabularReferenceTableAddSelectorOpenButton"
                  startIcon={<MdiIcon path="attachment-plus" />}
                  variant={'text'}
                  onClick={async () => {
                    await actions.activeVoteDefinitionsInResidentCityOpenAddSelectorAction!();
                  }}
                  disabled={editMode || !isFormUpdateable() || isLoading}
                >
                  {t(
                    'service.UserVoteDefinition.UserVoteDefinition_View_Edit.root.tabBar.ActiveVoteDefinitionsByOwnerResidentAreaGroup.tabBar.ActiveDebatesInResidentCityGroup.activeVoteDefinitionsInResidentCity.Add',
                    { defaultValue: 'Add' },
                  )}
                </Button>
              ) : null}
              {actions.activeVoteDefinitionsInResidentCityBulkRemoveAction && selectionModel.length > 0 ? (
                <Button
                  id="User/(esm/_yjuNkF5MEe6vsex_cZNQbQ)/TabularReferenceTableBulkRemoveButton"
                  startIcon={<MdiIcon path="link_off" />}
                  variant={'text'}
                  onClick={async () => {
                    const { result: bulkResult } = await actions.activeVoteDefinitionsInResidentCityBulkRemoveAction!(
