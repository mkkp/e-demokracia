...
export interface ServiceIssueCategoryIssueCategory_View_EditOwnerSetSelectorOwnerSetSelectorComponentActionDefinitions {
...
export interface ServiceIssueCategoryIssueCategory_View_EditOwnerSetSelectorOwnerSetSelectorComponentProps {
  uniqueId: string;
  actions: ServiceIssueCategoryIssueCategory_View_EditOwnerSetSelectorOwnerSetSelectorComponentActionDefinitions;
...
export const SERVICE_ISSUE_CATEGORY_ISSUE_CATEGORY_VIEW_EDIT_OWNER_SET_SELECTOR_OWNER_SET_SELECTOR_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY =
  'ServiceIssueCategoryIssueCategory_View_EditOwnerSetSelectorOwnerSetSelectorComponentSidekickComponent';

// XMIID: User/(esm/_8sinwIdgEe2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTable
// Name: owner::Set::Selector
export function ServiceIssueCategoryIssueCategory_View_EditOwnerSetSelectorOwnerSetSelectorComponent(
  props: ServiceIssueCategoryIssueCategory_View_EditOwnerSetSelectorOwnerSetSelectorComponentProps,
) {
  const {
    uniqueId,
    actions,
  ...
  const sidekickComponentFilter = `(&(${OBJECTCLASS}=${CUSTOM_VISUAL_ELEMENT_INTERFACE_KEY})(component=${SERVICE_ISSUE_CATEGORY_ISSUE_CATEGORY_VIEW_EDIT_OWNER_SET_SELECTOR_OWNER_SET_SELECTOR_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY}))`;
  const filterModelKey = `User/(esm/_8sinwIdgEe2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTable-${uniqueId}-${generatorUUIDv4}-filterModel`;
  const filtersKey = `User/(esm/_8sinwIdgEe2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTable-${uniqueId}-${generatorUUIDv4}-filters`;
  const rowsPerPageKey = `User/(esm/_8sinwIdgEe2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTable-${uniqueId}-${generatorUUIDv4}-rowsPerPage`;
  ...
  const representationColumn: GridColDef<ServiceServiceUserStored> = {
    ...
    headerName: t('service.IssueCategory.IssueCategory_View_Edit.owner.SetSelector.representation', {
  ...
  const effectiveTableColumns = useMemo(() => {
    const cols = [
      ...columns,
      ...columnsActionCalculator('User/(esm/_qUWS4GksEe25ONJ3V89cVA)/RelationType', rowActions, t, {
  ...
  const filterOptions = useMemo<FilterOption[]>(
    () => [
      {
        id: 'User/(esm/_iB-NIIdhEe2kLcMqsIbMgQ)/TableColumnFilter/(discriminator/User/(esm/_8sinwIdgEe2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTable)',
        attributeName: 'representation',
        label: t('service.IssueCategory.IssueCategory_View_Edit.owner.SetSelector.representation', {
          defaultValue: 'Representation',
        }) as string,
          ...
          Toolbar: () => (
            <GridToolbarContainer>
              {actions.filterAction && true ? (
                <Button
                  id="User/(esm/_8sinwIdgEe2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTableFilterButton"
                  startIcon={<MdiIcon path="filter" />}
                  variant={'text'}
                  onClick={async () => {
                    const filterResults = await actions.filterAction!(
                      'User/(esm/_8sinwIdgEe2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTableFilterButton',
                      filterOptions,
                      filterModel,
                      filters,
                    );
              ...
              {actions.selectorRangeAction && true ? (
                <Button
                  id="User/(esm/_8sinwIdgEe2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTableRefreshButton"
              ...
              {actions.openCreateFormAction && true ? (
                <Button
                  id="User/(esm/_8sinwIdgEe2kLcMqsIbMgQ)/TabularReferenceFieldLinkSetSelectorTableCreateButton"
