...
export interface ServiceServiceUserServiceUser_View_EditAreasActivityActivity_districtsActivityDistrictsAddSelectorActivityDistrictsAddSelectorComponentActionDefinitions {
...
export interface ServiceServiceUserServiceUser_View_EditAreasActivityActivity_districtsActivityDistrictsAddSelectorActivityDistrictsAddSelectorComponentProps {
  uniqueId: string;
  actions: ServiceServiceUserServiceUser_View_EditAreasActivityActivity_districtsActivityDistrictsAddSelectorActivityDistrictsAddSelectorComponentActionDefinitions;
...
export const SERVICE_SERVICE_USER_SERVICE_USER_VIEW_EDIT_AREAS_ACTIVITY_ACTIVITY_DISTRICTS_ACTIVITY_DISTRICTS_ADD_SELECTOR_ACTIVITY_DISTRICTS_ADD_SELECTOR_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY =
  'ServiceServiceUserServiceUser_View_EditAreasActivityActivity_districtsActivityDistrictsAddSelectorActivityDistrictsAddSelectorComponentSidekickComponent';

// XMIID: User/(esm/_I-9zEIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTable
// Name: activityDistricts::Add::Selector
export function ServiceServiceUserServiceUser_View_EditAreasActivityActivity_districtsActivityDistrictsAddSelectorActivityDistrictsAddSelectorComponent(
  props: ServiceServiceUserServiceUser_View_EditAreasActivityActivity_districtsActivityDistrictsAddSelectorActivityDistrictsAddSelectorComponentProps,
) {
  const {
    uniqueId,
    actions,
  ...
  const sidekickComponentFilter = `(&(${OBJECTCLASS}=${CUSTOM_VISUAL_ELEMENT_INTERFACE_KEY})(component=${SERVICE_SERVICE_USER_SERVICE_USER_VIEW_EDIT_AREAS_ACTIVITY_ACTIVITY_DISTRICTS_ACTIVITY_DISTRICTS_ADD_SELECTOR_ACTIVITY_DISTRICTS_ADD_SELECTOR_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY}))`;
  const filterModelKey = `User/(esm/_I-9zEIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTable-${uniqueId}-${generatorUUIDv4}-filterModel`;
  const filtersKey = `User/(esm/_I-9zEIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTable-${uniqueId}-${generatorUUIDv4}-filters`;
  const rowsPerPageKey = `User/(esm/_I-9zEIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTable-${uniqueId}-${generatorUUIDv4}-rowsPerPage`;
  ...
  const representationColumn: GridColDef<ServiceDistrictStored> = {
    ...
    headerName: t(
      'service.ServiceUser.ServiceUser_View_Edit.Areas.activity.activity_districts.activityDistricts.AddSelector.representation',
  ...
  const additionalToolbarActions: Record<string, ElementType> = actions?.activityDistrictsAdditionalToolbarButtons
    ? actions.activityDistrictsAdditionalToolbarButtons(data, isLoading, getSelectedRows(), clearSelections)
    : {};
  ...
  const filterOptions = useMemo<FilterOption[]>(
    () => [
      {
        id: 'User/(esm/_Mx494IdlEe2kLcMqsIbMgQ)/TableColumnFilter/(discriminator/User/(esm/_I-9zEIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTable)',
          ...
          Toolbar: () => (
            <GridToolbarContainer>
              {actions.filterAction && true ? (
                <Button
                  id="User/(esm/_I-9zEIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTableFilterButton"
                  startIcon={<MdiIcon path="filter" />}
                  variant={'text'}
                  onClick={async () => {
                    const filterResults = await actions.filterAction!(
                      'User/(esm/_I-9zEIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTableFilterButton',
                      filterOptions,
                      filterModel,
                      filters,
                    );
              ...
              {actions.selectorRangeAction && true ? (
                <Button
                  id="User/(esm/_I-9zEIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTableRefreshButton"
              ...
              {actions.openCreateFormAction && true ? (
                <Button
                  id="User/(esm/_I-9zEIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTableCreateButton"
