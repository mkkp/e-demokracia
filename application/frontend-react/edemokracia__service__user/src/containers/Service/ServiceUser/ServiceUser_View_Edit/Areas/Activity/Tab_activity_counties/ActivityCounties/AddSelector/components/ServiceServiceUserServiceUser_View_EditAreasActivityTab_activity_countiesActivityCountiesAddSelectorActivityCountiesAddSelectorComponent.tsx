...
export interface ServiceServiceUserServiceUser_View_EditAreasActivityTab_activity_countiesActivityCountiesAddSelectorActivityCountiesAddSelectorComponentActionDefinitions {
...
export interface ServiceServiceUserServiceUser_View_EditAreasActivityTab_activity_countiesActivityCountiesAddSelectorActivityCountiesAddSelectorComponentProps {
  uniqueId: string;
  actions: ServiceServiceUserServiceUser_View_EditAreasActivityTab_activity_countiesActivityCountiesAddSelectorActivityCountiesAddSelectorComponentActionDefinitions;
...
export const SERVICE_SERVICE_USER_SERVICE_USER_VIEW_EDIT_AREAS_ACTIVITY_TAB_ACTIVITY_COUNTIES_ACTIVITY_COUNTIES_ADD_SELECTOR_ACTIVITY_COUNTIES_ADD_SELECTOR_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY =
  'ServiceServiceUserServiceUser_View_EditAreasActivityTab_activity_countiesActivityCountiesAddSelectorActivityCountiesAddSelectorComponentSidekickComponent';

// XMIID: User/(esm/_I-t7cIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTable
// Name: activityCounties::Add::Selector
export function ServiceServiceUserServiceUser_View_EditAreasActivityTab_activity_countiesActivityCountiesAddSelectorActivityCountiesAddSelectorComponent(
  props: ServiceServiceUserServiceUser_View_EditAreasActivityTab_activity_countiesActivityCountiesAddSelectorActivityCountiesAddSelectorComponentProps,
) {
  const {
    uniqueId,
    actions,
  ...
  const sidekickComponentFilter = `(&(${OBJECTCLASS}=${CUSTOM_VISUAL_ELEMENT_INTERFACE_KEY})(component=${SERVICE_SERVICE_USER_SERVICE_USER_VIEW_EDIT_AREAS_ACTIVITY_TAB_ACTIVITY_COUNTIES_ACTIVITY_COUNTIES_ADD_SELECTOR_ACTIVITY_COUNTIES_ADD_SELECTOR_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY}))`;
  const filterModelKey = `User/(esm/_I-t7cIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTable-${uniqueId}-${generatorUUIDv4}-filterModel`;
  const filtersKey = `User/(esm/_I-t7cIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTable-${uniqueId}-${generatorUUIDv4}-filters`;
  const rowsPerPageKey = `User/(esm/_I-t7cIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTable-${uniqueId}-${generatorUUIDv4}-rowsPerPage`;
  ...
  const representationColumn: GridColDef<ServiceCountyStored> = {
    ...
    headerName: t(
      'service.ServiceUser.ServiceUser_View_Edit.Areas.activity.tab_activity_counties.activityCounties.AddSelector.representation',
      { defaultValue: 'County' },
    ) as string,
  ...
  const additionalToolbarActions: Record<string, ElementType> = actions?.activityCountiesAdditionalToolbarButtons
    ? actions.activityCountiesAdditionalToolbarButtons(data, isLoading, getSelectedRows(), clearSelections)
    : {};
  ...
  const filterOptions = useMemo<FilterOption[]>(
    () => [
      {
        id: 'User/(esm/_cnA48IdkEe2kLcMqsIbMgQ)/TableColumnFilter/(discriminator/User/(esm/_I-t7cIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTable)',
  ...
  return (
    <div
      ...
      data-table-name="activityCounties::Add::Selector"
    >
      <ComponentProxy
          ...
          Toolbar: () => (
            <GridToolbarContainer>
              {actions.filterAction && true ? (
                <Button
                  id="User/(esm/_I-t7cIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTableFilterButton"
                  startIcon={<MdiIcon path="filter" />}
                  variant={'text'}
                  onClick={async () => {
                    const filterResults = await actions.filterAction!(
                      'User/(esm/_I-t7cIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTableFilterButton',
                      filterOptions,
                      filterModel,
                      filters,
                    );
              ...
              {actions.selectorRangeAction && true ? (
                <Button
                  id="User/(esm/_I-t7cIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTableRefreshButton"
              ...
              {actions.openCreateFormAction && true ? (
                <Button
                  id="User/(esm/_I-t7cIXqEe2kLcMqsIbMgQ)/TabularReferenceFieldTableAddSelectorTableCreateButton"
