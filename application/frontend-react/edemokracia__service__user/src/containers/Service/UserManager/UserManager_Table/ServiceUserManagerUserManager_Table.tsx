...
export const SERVICE_USER_MANAGER_USER_MANAGER_TABLE_CONTAINER_ACTIONS_HOOK_INTERFACE_KEY =
  'SERVICE_USER_MANAGER_USER_MANAGER_TABLE_CONTAINER_ACTIONS_HOOK';
export type ServiceUserManagerUserManager_TableContainerHook =
  () => ServiceUserManagerUserManager_TableActionDefinitions;
...
export default function ServiceUserManagerUserManager_Table(props: ServiceUserManagerUserManager_TableProps) {
  ...
  const { service: customContainerHook } = useTrackService<ServiceUserManagerUserManager_TableContainerHook>(
    `(${OBJECTCLASS}=${SERVICE_USER_MANAGER_USER_MANAGER_TABLE_CONTAINER_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  const containerActions: ServiceUserManagerUserManager_TableActionDefinitions = customContainerHook?.() || {};
  const actions = useMemo(() => ({ ...containerActions, ...pageActions }), [containerActions, pageActions]);

  return (
    <Grid container>
      <Grid item data-name="UserManager_Table" xs={12} sm={12} md={36.0}>
        <Card id="User/(esm/_dGLZ0FvOEe6jm_SkPSYEYw)/TransferObjectTableVisualElement" data-name="UserManager_Table">
          <CardContent>
