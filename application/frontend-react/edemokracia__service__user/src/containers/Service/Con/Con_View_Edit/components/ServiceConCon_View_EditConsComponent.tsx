...
export const SERVICE_CON_CON_VIEW_EDIT_CONS_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY =
  'ServiceConCon_View_EditConsComponentSidekickComponent';
...
export function ServiceConCon_View_EditConsComponent(props: ServiceConCon_View_EditConsComponentProps) {
  ...
  const sidekickComponentFilter = `(&(${OBJECTCLASS}=${CUSTOM_VISUAL_ELEMENT_INTERFACE_KEY})(component=${SERVICE_CON_CON_VIEW_EDIT_CONS_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY}))`;
  const filterModelKey = `User/(esm/_WieK4IfYEe2u0fVmwtP5bA)/TabularReferenceFieldRelationDefinedTable-${uniqueId}-${generatorUUIDv4}-filterModel`;
  const filtersKey = `User/(esm/_WieK4IfYEe2u0fVmwtP5bA)/TabularReferenceFieldRelationDefinedTable-${uniqueId}-${generatorUUIDv4}-filters`;
  const rowsPerPageKey = `User/(esm/_WieK4IfYEe2u0fVmwtP5bA)/TabularReferenceFieldRelationDefinedTable-${uniqueId}-${generatorUUIDv4}-rowsPerPage`;
  ...
  const titleColumn: GridColDef<ServiceConStored> = {
    ...baseColumnConfig,
    field: 'title',
    headerName: t('service.Con.Con_View_Edit.cons.title', { defaultValue: 'Title' }) as string,
  ...
  const upVotesColumn: GridColDef<ServiceConStored> = {
    ...baseColumnConfig,
    field: 'upVotes',
    headerName: t('service.Con.Con_View_Edit.cons.upVotes', { defaultValue: 'up' }) as string,
    headerClassName: 'data-grid-column-header',
  ...
  const downVotesColumn: GridColDef<ServiceConStored> = {
    ...baseColumnConfig,
    field: 'downVotes',
    headerName: t('service.Con.Con_View_Edit.cons.downVotes', { defaultValue: 'down' }) as string,
    headerClassName: 'data-grid-column-header',
