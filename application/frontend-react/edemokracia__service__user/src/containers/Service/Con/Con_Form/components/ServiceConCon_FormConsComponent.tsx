...
export interface ServiceConCon_FormConsComponentActionDefinitions {
  serviceConCon_FormConsAddOpenSelector?: () => Promise<void>;
  serviceConCon_FormConsBulkDelete?: (selectedRows: ServiceConStored[]) => Promise<DialogResult<ServiceConStored[]>>;
  serviceConCon_FormConsBulkRemove?: (selectedRows: ServiceConStored[]) => Promise<DialogResult<ServiceConStored[]>>;
  serviceConCon_FormConsClear?: () => Promise<void>;
  serviceConCon_FormConsCreateOpen?: () => Promise<void>;
  serviceConCon_FormConsFilter?: (
    id: string,
    filterOptions: FilterOption[],
    model?: GridFilterModel,
    filters?: Filter[],
  ) => Promise<{ model?: GridFilterModel; filters?: Filter[] }>;
  serviceConCon_FormConsRefresh?: (queryCustomizer: ServiceConQueryCustomizer) => Promise<ServiceConStored[]>;
  ...
  serviceConCon_FormConsDelete?: (row: ServiceConStored, silentMode?: boolean) => Promise<void>;
  serviceConCon_FormConsRemove?: (row: ServiceConStored, silentMode?: boolean) => Promise<void>;
  serviceConCon_FormConsView?: (row: ServiceConStored) => Promise<void>;
...
export function ServiceConCon_FormConsComponent(props: ServiceConCon_FormConsComponentProps) {
  ...
  const rowActions: TableRowAction<ServiceConStored>[] = [
    {
      id: 'User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableRowRemoveButton',
      label: t('service.Con.Con.Form.service::Con::Con_Form::cons::Remove', { defaultValue: 'Remove' }) as string,
      icon: <MdiIcon path="link_off" />,
      disabled: (row: ServiceConStored) => isLoading,
      action: actions.serviceConCon_FormConsRemove
        ? async (rowData) => {
            await actions.serviceConCon_FormConsRemove!(rowData);
          }
        : undefined,
    },
    {
      id: 'User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableRowDeleteButton',
      label: t('service.Con.Con.Form.service::Con::Con_Form::cons::Delete', { defaultValue: 'Delete' }) as string,
      icon: <MdiIcon path="delete_forever" />,
      disabled: (row: ServiceConStored) => editMode || !row.__deleteable || isLoading,
      action: actions.serviceConCon_FormConsDelete
        ? async (rowData) => {
            await actions.serviceConCon_FormConsDelete!(rowData);
          }
        : undefined,
    },
    {
      id: 'User/(esm/_DBYxIHjsEe6cB8og8p0UuQ)/OperationFormTableRowCallOperationButton/(discriminator/User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableRowButtonGroup)',
    ...
    {
      id: 'User/(esm/_3sP2oIriEe2VSOmaAz6G9Q)/OperationFormTableRowCallOperationButton/(discriminator/User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableRowButtonGroup)',
    ...
    {
      id: 'User/(esm/_3sNaYIriEe2VSOmaAz6G9Q)/OperationFormTableRowCallOperationButton/(discriminator/User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableRowButtonGroup)',
    ...
    {
      id: 'User/(esm/_DBZYMHjsEe6cB8og8p0UuQ)/OperationFormTableRowCallOperationButton/(discriminator/User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableRowButtonGroup)',
  ...
  return (
    <>
      <StripedDataGrid
        ...
        onRowClick={
          actions.serviceConCon_FormConsView
            ? async (params: GridRowParams<ServiceConStored>) => await actions.serviceConCon_FormConsView!(params.row)
            : undefined
        }
        sortModel={sortModel}
        ...
        components={{
          Toolbar: () => (
            <GridToolbarContainer>
              {actions.serviceConCon_FormConsFilter && true ? (
                <Button
                  id="User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableFilterButton"
                  startIcon={<MdiIcon path="filter" />}
                  variant={'text'}
                  onClick={async () => {
                    const filterResults = await actions.serviceConCon_FormConsFilter!(
                  ...
                  disabled={isLoading}
                >
                  {t('service.Con.Con.Form.service::Con::Con_Form::cons::Filter', { defaultValue: 'Set Filters' })}
                  {filters.length ? ` (${filters.length})` : ''}
                </Button>
              ) : null}
              {actions.serviceConCon_FormConsRefresh && true ? (
                <Button
                  id="User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableRefreshButton"
                  startIcon={<MdiIcon path="refresh" />}
                  variant={'text'}
                  onClick={async () => {
                    await actions.serviceConCon_FormConsRefresh!(processQueryCustomizer(queryCustomizer));
                  }}
                  disabled={isLoading}
                >
                  {t('service.Con.Con.Form.service::Con::Con_Form::cons::Refresh', { defaultValue: 'Refresh' })}
                </Button>
              ) : null}
              {actions.serviceConCon_FormConsCreateOpen && true ? (
                <Button
                  id="User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableCreateButton"
                  startIcon={<MdiIcon path="note-add" />}
                  variant={'text'}
                  onClick={async () => {
                    await actions.serviceConCon_FormConsCreateOpen!();
                  }}
                  disabled={editMode || isLoading}
                >
                  {t('service.Con.Con.Form.service::Con::Con_Form::cons::Create', { defaultValue: 'Create' })}
                </Button>
              ) : null}
              {actions.serviceConCon_FormConsAddOpenSelector && true ? (
                <Button
                  id="User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableAddSelectorOpenButton"
                  startIcon={<MdiIcon path="attachment-plus" />}
                  variant={'text'}
                  onClick={async () => {
                    await actions.serviceConCon_FormConsAddOpenSelector!();
                  }}
                  disabled={isLoading}
                >
                  {t('service.Con.Con.Form.service::Con::Con_Form::cons::Add', { defaultValue: 'Add' })}
                </Button>
              ) : null}
              {actions.serviceConCon_FormConsClear && data.length ? (
                <Button
                  id="User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableClearButton"
                  startIcon={<MdiIcon path="link_off" />}
                  variant={'text'}
                  onClick={async () => {
                    await actions.serviceConCon_FormConsClear!();
                  }}
                  disabled={isLoading}
                >
                  {t('service.Con.Con.Form.service::Con::Con_Form::cons::Clear', { defaultValue: 'Clear' })}
                </Button>
              ) : null}
              {actions.serviceConCon_FormConsBulkRemove && selectionModel.length > 0 ? (
                <Button
                  id="User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableBulkRemoveButton"
                  startIcon={<MdiIcon path="link_off" />}
                  variant={'text'}
                  onClick={async () => {
                    const { result: bulkResult } = await actions.serviceConCon_FormConsBulkRemove!(
                  ...
                  disabled={isLoading}
                >
                  {t('service.Con.Con.Form.service::Con::Con_Form::cons::BulkRemove', { defaultValue: 'Remove' })}
                </Button>
              ) : null}
              {actions.serviceConCon_FormConsBulkDelete && selectionModel.length > 0 ? (
                <Button
                  id="User/(esm/_cI62AIfYEe2u0fVmwtP5bA)/TabularReferenceTableBulkDeleteButton"
                  startIcon={<MdiIcon path="delete_forever" />}
                  variant={'text'}
                  onClick={async () => {
                    const { result: bulkResult } = await actions.serviceConCon_FormConsBulkDelete!(
                      selectedRows.current,
                    );
                    if (bulkResult === 'submit') {
                      setSelectionModel([]); // not resetting on refreshes because refreshes would always remove selections...
                ...
                >
                  {t('service.Con.Con.Form.service::Con::Con_Form::cons::BulkDelete', { defaultValue: 'Delete' })}
                </Button>
              ) : null}
              <div>{/* Placeholder */}</div>
            </GridToolbarContainer>
