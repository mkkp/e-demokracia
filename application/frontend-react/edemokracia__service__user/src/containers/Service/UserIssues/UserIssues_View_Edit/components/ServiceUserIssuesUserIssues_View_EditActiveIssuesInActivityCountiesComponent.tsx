...
export interface ServiceUserIssuesUserIssues_View_EditActiveIssuesInActivityCountiesComponentActionDefinitions {
  ...
  getActiveIssuesInActivityCountiesMask?: () => string;
  ...
  activeIssuesInActivityCountiesOpenPageAction?: (row: ServiceIssueStored, isDraft?: boolean) => Promise<void>;
  activeIssuesInActivityCountiesAdditionalToolbarButtons?: (
...
export const SERVICE_USER_ISSUES_USER_ISSUES_VIEW_EDIT_ACTIVE_ISSUES_IN_ACTIVITY_COUNTIES_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY =
  'ServiceUserIssuesUserIssues_View_EditActiveIssuesInActivityCountiesComponentSidekickComponent';
...
export function ServiceUserIssuesUserIssues_View_EditActiveIssuesInActivityCountiesComponent(
  ...
  const sidekickComponentFilter = `(&(${OBJECTCLASS}=${CUSTOM_VISUAL_ELEMENT_INTERFACE_KEY})(component=${SERVICE_USER_ISSUES_USER_ISSUES_VIEW_EDIT_ACTIVE_ISSUES_IN_ACTIVITY_COUNTIES_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY}))`;
  const filterModelKey = `User/(esm/_7CQ7UFrXEe6gN-oVBDDIOQ)/TabularReferenceFieldRelationDefinedTable-${uniqueId}-${generatorUUIDv4}-filterModel`;
  const filtersKey = `User/(esm/_7CQ7UFrXEe6gN-oVBDDIOQ)/TabularReferenceFieldRelationDefinedTable-${uniqueId}-${generatorUUIDv4}-filters`;
  const rowsPerPageKey = `User/(esm/_7CQ7UFrXEe6gN-oVBDDIOQ)/TabularReferenceFieldRelationDefinedTable-${uniqueId}-${generatorUUIDv4}-rowsPerPage`;
  ...
  const [queryCustomizer, setQueryCustomizer] = useState<ServiceIssueQueryCustomizer>({
    _mask: '{countyRepresentation,created,status,title}',
    _seek: {
      limit: rowsPerPage + 1,
    },
  ...
  const countyRepresentationColumn: GridColDef<ServiceIssueStored> = {
    ...
    headerName: t('service.UserIssues.UserIssues_View_Edit.activeIssuesInActivityCounties.countyRepresentation', {
  ...
  const titleColumn: GridColDef<ServiceIssueStored> = {
    ...baseColumnConfig,
    field: 'title',
    headerName: t('service.UserIssues.UserIssues_View_Edit.activeIssuesInActivityCounties.title', {
      defaultValue: 'Title',
    }) as string,
    headerClassName: 'data-grid-column-header',
  ...
  const createdColumn: GridColDef<ServiceIssueStored> = {
    ...baseColumnConfig,
    field: 'created',
    headerName: t('service.UserIssues.UserIssues_View_Edit.activeIssuesInActivityCounties.created', {
  ...
  const statusColumn: GridColDef<ServiceIssueStored> = {
    ...baseColumnConfig,
    field: 'status',
    headerName: t('service.UserIssues.UserIssues_View_Edit.activeIssuesInActivityCounties.status', {
      defaultValue: 'Status',
    }) as string,
    headerClassName: 'data-grid-column-header',
  ...
  const columns = useMemo<GridColDef<ServiceIssueStored>[]>(
    () => [countyRepresentationColumn, titleColumn, createdColumn, statusColumn],
    [l10nLocale],
  );
  ...
  const rowActions: TableRowAction<ServiceIssueStored>[] = useMemo(
    () => [
      ...
      {
        id: 'User/(esm/_qJPPC3jvEe6cB8og8p0UuQ)/OperationFormTableRowCallOperationButton/(discriminator/User/(esm/_7CQ7UFrXEe6gN-oVBDDIOQ)/TabularReferenceTableRowButtonGroup)',
      ...
      {
        id: 'User/(esm/_qJPPA3jvEe6cB8og8p0UuQ)/OperationFormTableRowCallOperationButton/(discriminator/User/(esm/_7CQ7UFrXEe6gN-oVBDDIOQ)/TabularReferenceTableRowButtonGroup)',
  ...
  const additionalToolbarActions: Record<string, ElementType> =
    actions?.activeIssuesInActivityCountiesAdditionalToolbarButtons
      ? actions.activeIssuesInActivityCountiesAdditionalToolbarButtons(
          data,
          isLoading,
          getSelectedRows(),
  ...
  const filterOptions = useMemo<FilterOption[]>(
    () => [
      {
        id: 'User/(esm/_jUVwUFw-Ee6gN-oVBDDIOQ)/TableColumnFilter/(discriminator/User/(esm/_7CQ7UFrXEe6gN-oVBDDIOQ)/TabularReferenceFieldRelationDefinedTable)',
      ...
      {
        id: 'User/(esm/_GQdN4FrfEe6gN-oVBDDIOQ)/TableColumnFilter/(discriminator/User/(esm/_7CQ7UFrXEe6gN-oVBDDIOQ)/TabularReferenceFieldRelationDefinedTable)',
        attributeName: 'title',
      ...
      {
        id: 'User/(esm/_IyBLsFrfEe6gN-oVBDDIOQ)/TableColumnFilter/(discriminator/User/(esm/_7CQ7UFrXEe6gN-oVBDDIOQ)/TabularReferenceFieldRelationDefinedTable)',
      ...
      {
        id: 'User/(esm/_IyBywVrfEe6gN-oVBDDIOQ)/TableColumnFilter/(discriminator/User/(esm/_7CQ7UFrXEe6gN-oVBDDIOQ)/TabularReferenceFieldRelationDefinedTable)',
    ...
    if (ownerData.__signedIdentifier) {
      ...
      try {
        const processedQueryCustomizer = {
          ...
          _mask: actions.getActiveIssuesInActivityCountiesMask
            ? actions.getActiveIssuesInActivityCountiesMask()
            : queryCustomizer._mask,
        };
        const { data: res, headers } =
          await actions.activeIssuesInActivityCountiesRefreshAction!(processedQueryCustomizer);
        ...
        onRowClick={
          actions.activeIssuesInActivityCountiesOpenPageAction
            ? async (params: GridRowParams<ServiceIssueStored>) =>
                await actions.activeIssuesInActivityCountiesOpenPageAction!(params.row, false)
            : undefined
        }
        sortModel={sortModel}
