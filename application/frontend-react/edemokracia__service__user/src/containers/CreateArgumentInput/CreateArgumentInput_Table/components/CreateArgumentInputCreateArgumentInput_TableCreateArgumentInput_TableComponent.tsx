...
export interface CreateArgumentInputCreateArgumentInput_TableCreateArgumentInput_TableComponentActionDefinitions {
  ...
  openPageAction?: (row: CreateArgumentInputStored, isDraft?: boolean) => Promise<void>;
...
export const CREATE_ARGUMENT_INPUT_CREATE_ARGUMENT_INPUT_TABLE_CREATE_ARGUMENT_INPUT_TABLE_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY =
  'CreateArgumentInputCreateArgumentInput_TableCreateArgumentInput_TableComponentSidekickComponent';
...
export function CreateArgumentInputCreateArgumentInput_TableCreateArgumentInput_TableComponent(
  ...
  const sidekickComponentFilter = `(&(${OBJECTCLASS}=${CUSTOM_VISUAL_ELEMENT_INTERFACE_KEY})(component=${CREATE_ARGUMENT_INPUT_CREATE_ARGUMENT_INPUT_TABLE_CREATE_ARGUMENT_INPUT_TABLE_COMPONENT_SIDEKICK_COMPONENT_INTERFACE_KEY}))`;
  const filterModelKey = `User/(esm/_Ga4NMHW5Ee2LTNnGda5kaw)/TransferObjectTableTable-${uniqueId}-${generatorUUIDv4}-filterModel`;
  const filtersKey = `User/(esm/_Ga4NMHW5Ee2LTNnGda5kaw)/TransferObjectTableTable-${uniqueId}-${generatorUUIDv4}-filters`;
  const rowsPerPageKey = `User/(esm/_Ga4NMHW5Ee2LTNnGda5kaw)/TransferObjectTableTable-${uniqueId}-${generatorUUIDv4}-rowsPerPage`;
  ...
  const SidekickComponent =
    useTrackComponent<FC<SidekickComponentProps<CreateArgumentInputStored>>>(sidekickComponentFilter);

  const isLoading = useMemo(() => isInternalLoading || !!isOwnerLoading, [isInternalLoading, isOwnerLoading]);

  const selectedRows = useRef<CreateArgumentInputStored[]>([]);

  const titleColumn: GridColDef<CreateArgumentInputStored> = {
    ...baseColumnConfig,
    field: 'title',
  ...
  const descriptionColumn: GridColDef<CreateArgumentInputStored> = {
    ...baseColumnConfig,
    field: 'description',
  ...
  const columns = useMemo<GridColDef<CreateArgumentInputStored>[]>(() => [titleColumn, descriptionColumn], []);

  const rowActions: TableRowAction<CreateArgumentInputStored>[] = useMemo(
    () => [
      {
  ...
  const getSelectedRows: () => CreateArgumentInputStored[] = () => {
          ...
          Toolbar: () => (
            <GridToolbarContainer>
              ...
              {actions.exportAction && true ? (
                <Button
                  id="User/(esm/_Ga4NMHW5Ee2LTNnGda5kaw)/TransferObjectTableExportButton"
