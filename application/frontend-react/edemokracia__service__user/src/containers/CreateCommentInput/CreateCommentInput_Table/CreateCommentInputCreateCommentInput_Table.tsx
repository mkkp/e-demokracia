...
export const CREATE_COMMENT_INPUT_CREATE_COMMENT_INPUT_TABLE_CONTAINER_ACTIONS_HOOK_INTERFACE_KEY =
  'CREATE_COMMENT_INPUT_CREATE_COMMENT_INPUT_TABLE_CONTAINER_ACTIONS_HOOK';
export type CreateCommentInputCreateCommentInput_TableContainerHook =
  () => CreateCommentInputCreateCommentInput_TableActionDefinitions;
...
export default function CreateCommentInputCreateCommentInput_Table(
  ...
  const { service: customContainerHook } = useTrackService<CreateCommentInputCreateCommentInput_TableContainerHook>(
    `(${OBJECTCLASS}=${CREATE_COMMENT_INPUT_CREATE_COMMENT_INPUT_TABLE_CONTAINER_ACTIONS_HOOK_INTERFACE_KEY})`,
  );
  const containerActions: CreateCommentInputCreateCommentInput_TableActionDefinitions = customContainerHook?.() || {};
  const actions = useMemo(() => ({ ...pageActions, ...containerActions }), [pageActions, containerActions]);

  return (
    <Grid container>
      <Grid item data-name="CreateCommentInput_Table" xs={12} sm={12} md={36.0}>
        <Card
          id="User/(esm/_kYeCcIe5Ee2kLcMqsIbMgQ)/TransferObjectTableVisualElement"
          data-name="CreateCommentInput_Table"
        >
          <CardContent>
